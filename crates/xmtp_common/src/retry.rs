...
mod derive_tests {
    ...
    #[test]
    fn default_true_from_variant_must_be_explicit() {
        // Under `default = true`, a #[from] variant must state its own policy —
        // an unannotated one is a compile error (see the trybuild fixture
        // `default_true_unannotated_from.rs`). `#[retry(true)]` restores the old
        // blanket baseline: a constant `true` that does NOT forward.
        #[derive(Debug, Error, Retryable)]
        #[retry(default = true)]
        enum E {
            #[error(transparent)]
