...
pub struct StreamSettings {
    ...
    pub default_consumer_lease_duration: Duration,
    /// Healthy-primary wait before an absent exact identity reference is rejected.
    pub identity_reference_wait: Duration,
...
#[error("invalid stream setting: {0}")]
pub struct InvalidStreamSettings(pub &'static str);

impl StreamSettings {
    /// Reject zero, overflowing, or inconsistent limits before starting client work.
    pub fn validate(&self) -> Result<(), InvalidStreamSettings> {
        let positive = [
        ...
        for (name, value) in positive {
            if value == 0 || value > i64::MAX as u64 {
                return Err(InvalidStreamSettings(name));
            }
        }
        for (name, value) in [
            (
        ...
        if self.active_database_poll_interval >= self.default_consumer_lease_duration {
            return Err(InvalidStreamSettings("default_consumer_lease_duration"));
        }
        if self.identity_reference_wait
            <= Duration::from_millis(BACKEND_DEFAULT_STATEMENT_TIMEOUT_MS)
        {
            return Err(InvalidStreamSettings("identity_reference_wait"));
        }
        Ok(())
    }
