...
pub(crate) mod tests {
    ...
    #[xmtp_common::test]
    fn test_client_error_retryable_golden() {
        use xmtp_common::RetryableError;

        // Previously: Generic(err) => err.contains("database is locked")
        assert!(super::ClientError::Generic("database is locked".to_string()).is_retryable());
        assert!(!super::ClientError::Generic("some other failure".to_string()).is_retryable());
        // Previously: EnvelopesNotYetVisible => true
        assert!(super::ClientError::EnvelopesNotYetVisible { node_id: 1 }.is_retryable());
        // Previously: catch-all false (AlreadyClosed not in the retryable set)
        assert!(!super::ClientError::AlreadyClosed.is_retryable());
    }
