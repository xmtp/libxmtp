...
mod retryable_golden_tests {
    ...
    #[test]
    fn identity_error_retryability() {
        // Previously: ApiClient | StorageError | OpenMlsStorageError => forward; _ => false
        // Forwarding variant with a retryable inner is retryable.
        assert!(
            IdentityError::OpenMlsStorageError(SqlKeyStoreError::Connection(
                ConnectionError::DisconnectInTransaction
            ))
            .is_retryable()
        );
        // `Db` was hardcoded non-retryable even though ConnectionError itself can be retryable.
        assert!(!IdentityError::Db(ConnectionError::DisconnectInTransaction).is_retryable());
        // A plain terminal variant is never retryable.
        assert!(!IdentityError::Bincode.is_retryable());
    }
}
