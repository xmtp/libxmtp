...
use crate::{
    groups::{
        ...
        intents::{
            CommitPendingProposalsIntentData, ProposeAddMembersIntentData,
            ProposeRemoveMembersIntentData,
        },
...
/// Test that all_members_support_proposals_unchecked returns true (test utility).
#[xmtp_common::test(unwrap_try = true)]
async fn test_all_members_support_proposals_unchecked() {
    tester!(alix);
    tester!(bo);
    ...
    let supports_proposals = alix_group
        .load_mls_group_with_lock_async(async |mls_group| {
            Ok::<bool, crate::groups::GroupError>(
                alix_group.all_members_support_proposals_unchecked(&mls_group),
            )
        })
        .await?;
    ...
    assert!(
        supports_proposals,
        "all_members_support_proposals_unchecked should always return true"
    );
}
...
/// Test that proposal intents can be created, queued, and deserialized correctly.
#[rstest]
#[case::add_single(IntentKind::ProposeAddMembers, vec!["inbox1".to_string()])]
#[case::add_multiple(IntentKind::ProposeAddMembers, vec!["inbox1".to_string(), "inbox2".to_string(), "inbox3".to_string()])]
#[case::add_empty(IntentKind::ProposeAddMembers, vec![])]
#[case::remove_single(IntentKind::ProposeRemoveMembers, vec!["inbox1".to_string()])]
#[case::remove_multiple(IntentKind::ProposeRemoveMembers, vec!["inbox1".to_string(), "inbox2".to_string()])]
#[xmtp_common::test]
async fn test_proposal_intent_serialization(
    #[case] kind: IntentKind,
    #[case] inbox_ids: Vec<String>,
) {
    tester!(alix);
    tester!(bo);
    ...
    let intent_bytes: Vec<u8> = match kind {
        IntentKind::ProposeAddMembers => ProposeAddMembersIntentData::new(inbox_ids.clone()).into(),
        IntentKind::ProposeRemoveMembers => {
            ProposeRemoveMembersIntentData::new(inbox_ids.clone()).into()
        }
        _ => panic!("Unexpected intent kind"),
    };
    ...
    match kind {
        IntentKind::ProposeAddMembers => {
            let parsed = ProposeAddMembersIntentData::try_from(intent.data.as_slice()).unwrap();
            assert_eq!(parsed.inbox_ids, inbox_ids);
        }
        IntentKind::ProposeRemoveMembers => {
            let parsed = ProposeRemoveMembersIntentData::try_from(intent.data.as_slice()).unwrap();
            assert_eq!(parsed.inbox_ids, inbox_ids);
        }
        _ => {}
    }
...
async fn test_e2e_propose_add_member_flow() {
    ...
    // 2. Alix proposes to add caro
    let intent_data = ProposeAddMembersIntentData::new(vec![caro.inbox_id().to_string()]);
...
async fn test_e2e_propose_remove_member_flow() {
    ...
    // 2. Alix proposes to remove caro
    let intent_data = ProposeRemoveMembersIntentData::new(vec![caro.inbox_id().to_string()]);
    let alix_db = alix_group.context.db();
    let propose_intent =
        alix_db.insert_group_intent(xmtp_db::group_intent::NewGroupIntent::new(
            IntentKind::ProposeRemoveMembers,
...
async fn test_propose_invalid_member_operations(#[case] is_add: bool) {
    ...
    let (kind, intent_bytes) = if is_add {
        ...
        (
            IntentKind::ProposeAddMembers,
            ProposeAddMembersIntentData::new(vec![bo.inbox_id().to_string()]).into(),
        )
    } else {
        // Try to remove caro who is not in the group
        (
            IntentKind::ProposeRemoveMembers,
            ProposeRemoveMembersIntentData::new(vec![caro.inbox_id().to_string()]).into(),
        )
    };
...
async fn test_message_auto_commits_pending_proposals() {
    ...
    let propose_intent = db.insert_group_intent(xmtp_db::group_intent::NewGroupIntent::new(
        IntentKind::ProposeAddMembers,
        alix_group.group_id.clone(),
        ProposeAddMembersIntentData::new(vec![caro.inbox_id().to_string()]).into(),
        false,
    ))?;
    ...
    assert!(caro_is_member, "Caro should be a group member after auto-commit");
}
...
async fn test_multiple_add_proposals_before_commit() {
    ...
    let propose_dave = alix_db.insert_group_intent(xmtp_db::group_intent::NewGroupIntent::new(
        IntentKind::ProposeAddMembers,
        alix_group.group_id.clone(),
        ProposeAddMembersIntentData::new(vec![dave.inbox_id().to_string()]).into(),
        false,
    ))?;
    alix_group
...
async fn test_concurrent_proposals_from_different_members() {
    ...
    let bo_propose = bo_db.insert_group_intent(xmtp_db::group_intent::NewGroupIntent::new(
        IntentKind::ProposeAddMembers,
        bo_group.group_id.clone(),
        ProposeAddMembersIntentData::new(vec![eve.inbox_id().to_string()]).into(),
        false,
    ))?;
