...
async fn test_request_readd() {
    ...
    let b_group = bo.group(&group.group_id).await.unwrap();
    b_group
        .update_consent_state(ConsentState::Allowed)
        .await
        .unwrap();
    let c_group = caro.group(&group.group_id).await.unwrap();
    c_group
        .update_consent_state(ConsentState::Allowed)
        .await
        .unwrap();
...
async fn test_request_readd_dm() {
    ...
    let dm = alix
        ...
        .unwrap();
    bo.sync_all_welcomes_and_groups(None).await.unwrap();
    let bo_dm = bo.group(&dm.group_id).await.unwrap();
    bo_dm
        .update_consent_state(ConsentState::Allowed)
        .await
        .unwrap();
...
async fn test_readd_installation_succeeds() {
    ...
    bo.sync_all_welcomes_and_groups(None).await.unwrap();
    caro.sync_all_welcomes_and_groups(None).await.unwrap();
    let b_group = bo.group(&a_group.group_id).await.unwrap();
    let c_group = caro.group(&a_group.group_id).await.unwrap();
...
async fn test_readd_bookkeeping() {
    ...
    c_group
        ...
        .await
        .unwrap();
    let d_group = devon.group(&group.group_id).await.unwrap();
    d_group
        .update_consent_state(ConsentState::Allowed)
        .await
        .unwrap();
...
async fn test_request_readd_with_allowlisted_groups() {
    ...
    let a_group = alix.group(&group_id).await.unwrap();
    a_group
        .update_consent_state(ConsentState::Allowed)
        .await
        .unwrap();
