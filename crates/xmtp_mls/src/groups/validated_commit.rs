...
impl ExpectedDiff {
    ...
    async fn extract_expected_diff_with_proposers(
        ...
        // 1. The original Add proposal proposers (the member who actually proposed the add)
        // 2. The GCE proposer (if membership changed via GCE proposal without a direct Add proposal)
        let added_inboxes = membership_diff
            .added_inboxes
            .iter()
            .map(|inbox_id| {
                // Prefer the original Add proposal proposer over the GCE proposer.
                // When committing pending proposals, the committer creates the GCE proposal,
                // but the original proposer's permissions should be checked.
                let proposer = added_installation_proposers
                    .values()
                    .next()
                    .cloned()
        ...
        // 1. The original Remove proposal proposers (the member who actually proposed the removal)
        ...
        let removed_inboxes = membership_diff
            ...
            .iter()
            .map(|inbox_id| {
                let proposer = removed_installation_proposers
                    .values()
                    .next()
                    .cloned()
                    .or_else(|| gce_proposer.clone());
