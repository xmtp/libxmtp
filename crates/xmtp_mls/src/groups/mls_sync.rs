use super::{
    ...
    intents::{
        ...
        ProposeMemberUpdateIntentData, ProposeGroupContextExtensionsIntentData,
        SendMessageIntentData, SendWelcomesAction,
    ...
    Context: XmtpSharedContext,
{
                ...
                match result {
                    ...
                    Ok(Some(PublishIntentData {
                        ...
                        if has_staged_commit {
                            log_event!(Event::GroupSyncStagedCommitPresent, self.context.installation_id(), group_id = intent.group_id.short_hex());
                            return Ok(());
                        }
                    }
                    Ok(None) => {
    ...
    async fn get_publish_intent_data(
        ...
        match intent.kind {
            ...
            IntentKind::ProposeMemberUpdate => {
                let intent_data =
                    ProposeMemberUpdateIntentData::try_from(intent.data.as_slice())?;
                let group_epoch = openmls_group.epoch().as_u64();
                let signer = &self.context.identity().installation_keys;
                ...
                if !intent_data.add_inbox_ids.is_empty() {
                    ...
                    let inbox_ids_to_add: Vec<&str> =
                        intent_data.add_inbox_ids.iter().map(|s| s.as_str()).collect();
