...
pub enum GroupError {
    ...
    /// its registration hasn't propagated yet. Retrying re-runs the
    /// fetch, so the miss is transient; classifying it non-retryable
    /// permanently failed sync-group membership adds that raced a new
    /// installation's identity propagation. Retryable.
