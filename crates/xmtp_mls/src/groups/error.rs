...
mod retryable_golden_tests {
    //! Golden tests pinning the retryability of the error enums in this file
    //! after their migration to `#[derive(Retryable)]`.
    use super::*;

    #[xmtp_common::test]
    fn group_error_hardcoded_true_variants() {
        // Previously: `Self::LockUnavailable => true,`
        assert!(GroupError::LockUnavailable.is_retryable());
        // Previously: `Self::SyncFailedToWait(_) => true,`
        assert!(GroupError::SyncFailedToWait(Box::default()).is_retryable());
    }

    #[xmtp_common::test]
    fn group_error_inherits_storage() {
        // Previously: `Self::Storage(storage) => storage.is_retryable(),`
        let retryable = xmtp_db::StorageError::DieselConnect(
            diesel::ConnectionError::BadConnection("nope".into()),
        );
        assert!(GroupError::Storage(retryable).is_retryable());
        assert!(!GroupError::Storage(xmtp_db::StorageError::IntentionalRollback).is_retryable());
    }

    #[xmtp_common::test]
    fn group_error_non_retryable_variants() {
        // Previously: grouped in the `=> false` arm.
        assert!(!GroupError::UserLimitExceeded.is_retryable());
        assert!(!GroupError::GroupInactive.is_retryable());
        assert!(!GroupError::MissingPendingCommit.is_retryable());
    }

    #[xmtp_common::test]
    fn flat_enums_are_never_retryable() {
        // Previously: `impl RetryableError for DeleteMessageError { .. false }`
        assert!(!DeleteMessageError::NotAuthorized.is_retryable());
        // Previously: `impl RetryableError for MetadataPermissionsError { .. false }`
        assert!(!MetadataPermissionsError::InvalidPermissionUpdate.is_retryable());
        // Previously: `impl RetryableError for GroupLeaveValidationError { .. false }`
        assert!(!GroupLeaveValidationError::DmLeaveForbidden.is_retryable());
        // Previously: `impl RetryableError for DmValidationError { .. false }`
        assert!(!DmValidationError::InvalidPermissions.is_retryable());
    }
}
