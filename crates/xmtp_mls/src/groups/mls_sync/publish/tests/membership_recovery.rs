...
async fn non_admin_commits_valid_subset_without_resigning_admin_add(
    ...
    assert_eq!(staged.add_proposals().count(), usize::from(!omitted));
...
async fn rejected_removal_with_new_installation_does_not_block_publication(
    ...
    let fetched = bo2
        .context
        .api()
        .fetch_key_packages(&[bo2.installation_id])
        .await?;
    let current_package = xmtp_id::key_package::VerifiedKeyPackageV2::from_bytes(
        &openmls_rust_crypto::RustCrypto::default(),
        &fetched[&bo2.installation_id]
            .as_ref()
            .unwrap()
