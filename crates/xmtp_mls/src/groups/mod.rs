    ...
    Context: XmtpSharedContext,
{
    ...
    /// field-coupling invariants and queued as-is. Prefer
    /// [`MlsGroup::enable_external_commits`] /
    /// [`MlsGroup::revoke_external_commits`], which mint fresh invite
    /// coordinates and preserve durable settings.
    ///
    ...
    pub async fn set_external_commit_policy(
        ...
        self.queue_external_commit_policy(policy).await
    }

    /// Encode + queue an `EXTERNAL_COMMIT_POLICY` write as a
    /// single-component `AppDataUpdate` intent.
    async fn queue_external_commit_policy(
        ...
        let intent_data = crate::groups::intents::AppDataUpdateIntentData::new(
    ...
    /// A single-component `EXTERNAL_COMMIT_POLICY` write carrying:
    ///
    ...
    ///
    /// Per XIP-82, the switch itself authorizes the joiner's
    /// `GROUP_MEMBERSHIP` self-entry insert — no separate registry
    /// grant is written.
    ///
