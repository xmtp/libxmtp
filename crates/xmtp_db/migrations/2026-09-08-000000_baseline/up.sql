...
, next_key_package_rotation_ns BIGINT, registration_cursor_sequence_id BIGINT);
...
, sequence_id bigint, rotated_at_ns BIGINT NOT NULL DEFAULT 0, conversation_type INTEGER NOT NULL DEFAULT 1, dm_id TEXT, last_message_ns BIGINT, message_disappear_from_ns  BIGINT, message_disappear_in_ns BIGINT, paused_for_version TEXT DEFAULT NULL, maybe_forked BOOLEAN NOT NULL DEFAULT FALSE, fork_details TEXT NOT NULL DEFAULT '', should_publish_commit_log BOOLEAN NOT NULL DEFAULT FALSE, commit_log_public_key BLOB, is_commit_log_forked BOOLEAN, has_pending_leave_request BOOLEAN);
...
CREATE TABLE group_welcome_discovery (
    ...
    first_welcome_sequence_id BIGINT NOT NULL CHECK (first_welcome_sequence_id > 0)
);

CREATE INDEX group_welcome_discovery_sequence
    ON group_welcome_discovery(first_welcome_sequence_id, group_id);
...
CREATE TABLE group_intents(
    ...
    "publish_attempts" int NOT NULL DEFAULT 0, staged_commit BLOB, published_in_epoch BIGINT, should_push BOOLEAN NOT NULL DEFAULT TRUE, sequence_id BIGINT,
    prepared_envelopes BLOB,
...
, delete_at_ns BIGINT, post_quantum_public_key BLOB,
    published_sequence_id BIGINT CHECK (published_sequence_id IS NULL OR published_sequence_id > 0));
...
, dm_group_updates_migrated BOOL NOT NULL DEFAULT FALSE,
...
CREATE TABLE refresh_state (
    ...
    received_sequence_id BIGINT CHECK (received_sequence_id IS NULL OR received_sequence_id >= sequence_id),
    last_rejection_sequence_id BIGINT CHECK (last_rejection_sequence_id IS NULL OR (last_rejection_sequence_id > 0 AND last_rejection_sequence_id <= received_sequence_id)),
    last_rejection_code TEXT,
    CHECK ((last_rejection_sequence_id IS NULL) = (last_rejection_code IS NULL)),
    PRIMARY KEY (entity_id, entity_kind)
);

CREATE TABLE incoming_envelopes (
    entity_id BLOB NOT NULL,
    entity_kind INTEGER NOT NULL CHECK (entity_kind IN (1, 2, 8)),
    sequence_id BIGINT NOT NULL CHECK (sequence_id > 0),
    envelope BLOB NOT NULL,
    retry_at_ns BIGINT NOT NULL DEFAULT 0,
    blocked BOOL NOT NULL DEFAULT FALSE CHECK (blocked IN (0, 1)),
    error_code TEXT,
    retry_expires_at_ns BIGINT,
    PRIMARY KEY (entity_id, entity_kind, sequence_id),
    FOREIGN KEY (entity_id, entity_kind) REFERENCES refresh_state(entity_id, entity_kind)
) WITHOUT ROWID;

CREATE INDEX incoming_welcomes_ready
    ON incoming_envelopes(blocked, retry_at_ns, sequence_id) WHERE entity_kind = 1;

INSERT INTO refresh_state(entity_id, entity_kind, sequence_id) VALUES (x'', 9, 0);
...
CREATE UNIQUE INDEX group_messages_delivery_sequence ON group_messages(delivery_sequence)
    WHERE delivery_sequence IS NOT NULL;
CREATE INDEX group_messages_group_delivery_sequence ON group_messages(group_id, delivery_sequence)
    WHERE delivery_sequence IS NOT NULL;
