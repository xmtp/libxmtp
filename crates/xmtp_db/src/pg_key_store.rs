//! The Postgres OpenMLS key store: [`PgKeyStore`].
//!
//! The sqlx/Postgres counterpart of the SQLite-backend
...
//!
//! Servers only; never wasm. Gated behind the `sqlx` feature.
...
/// `deserialize_any`/untagged forms). The SQLite backend serializes with
/// bincode; moving it to CBOR needs a data migration and is deferred.
#[derive(Default)]
...
/// The Postgres-backend MLS key store: an OpenMLS [`StorageProvider`] backed by a
...
impl PgKeyStore {
    ...
    async fn typed_select(
        ...
        let row: Option<(Vec<u8>,)> = sqlx::query_as(&format!(
            "SELECT {value_col} FROM {table} WHERE {key_col} = $1"
        ))
        .bind(key.to_vec())
...
impl StorageProvider<CURRENT_VERSION> for PgKeyStore {
    ...
    >(
        ...
        let mut conn = self.db.conn().await?;
        PostgresStorageProvider::<CborCodec>::new(&mut conn)
            .write_encryption_epoch_key_pairs::<GroupId, EpochKey, HpkeKeyPair>(
                group_id, epoch, leaf_index, key_pairs,
            )
            .await
            .map_err(|e| SqlKeyStoreError::from(crate::ConnectionError::from(e)))
    }
    ...
    >(
        ...
        let mut conn = self.db.conn().await?;
        PostgresStorageProvider::<CborCodec>::new(&mut conn)
            .encryption_epoch_key_pairs::<GroupId, EpochKey, HpkeKeyPair>(
...
impl XmtpMlsStorageProvider for PgKeyStore {
    ...
    // Not `async fn`: the returned future must be explicitly `+ MaybeSend` so it can
    // flow into spawned Send workers, which `async fn` would not guarantee.
