    ...
    T: QueryGroupIntent + xmtp_common::MaybeSync,
{
    ...
    async fn set_group_intent_superseded(&self, intent_id: ID) -> Result<(), StorageError> {
        (**self).set_group_intent_superseded(intent_id).await
    }
...
mod pg_impl {
    ...
    impl<C: crate::PgConnectionProvider> crate::Fetch<StoredGroupIntent> for C {
        type Key = ID;
        async fn fetch(
            &self,
            key: &Self::Key,
        ) -> Result<Option<StoredGroupIntent>, crate::StorageError> {
            let sql = format!(
                "SELECT {} FROM group_intents WHERE id = $1 LIMIT 1",
    ...
    impl QueryGroupIntent for PgDb {
        ...
        async fn set_group_intent_superseded(&self, intent_id: ID) -> Result<(), StorageError> {
            let mut c = self.conn().await?;
            let rows_changed =
                sqlx::query("UPDATE group_intents SET state = $2 WHERE id = $1 AND state = $3")
                    .bind(intent_id)
                    .bind(IntentState::Superseded)
                    // The state machine allows only ToPublish -> Superseded.
                    .bind(IntentState::ToPublish)
                    .execute(&mut *c)
                    .await
                    .map_err(crate::ConnectionError::from)?
