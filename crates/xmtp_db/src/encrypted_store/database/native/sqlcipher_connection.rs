    ...
    fn validate(&self, conn: &mut SqliteConnection) -> Result<(), PlatformStorageError> {
        ...
        .map_err(|e| {
            ...
            if msg.contains("malformed") || msg.contains("corrupt") {
                ...
                // Everything else stays classified as a key problem. Note:
                // because we always use a plaintext header, a wrong key does
                // NOT reliably produce one fixed error string — decrypting
                // with a bad key yields garbage pages whose failure shape is
                // data-dependent (see IntegrityCheckResult::Unreadable's
                // "indistinguishable by design" note in integrity.rs). A
                // wrong key can therefore still surface as DatabaseCorrupt
                // above; both variants are non-retryable, so only the
