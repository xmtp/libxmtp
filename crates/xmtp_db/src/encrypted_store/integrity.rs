...
pub(crate) fn classify_check_error(e: diesel::result::Error) -> IntegrityCheckResult {
    ...
    if lower.contains("malformed") || lower.contains("corrupt") {
        ...
        // "not a database" is the classic SQLITE_NOTADB text. With a plaintext
        // header (used for all SQLCipher databases here) SQLite's own format
        // check on open passes even with the wrong key or a corrupted header,
        // so the eventual page-decrypt/HMAC failure often surfaces instead as
        // the bare SQLite fallback text "SQL logic error" (SQLITE_ERROR with
        // no elaboration) once a check query tries to read the page. Both
        // shapes are the same "can't read this page with this key" failure —
        // wrong key or mangled header/ciphertext, indistinguishable by
        // design (see `Unreadable`'s doc comment).
...
/// SQLite decodes %XX escapes in `file:` URI filenames, so escape the three
/// characters that would otherwise terminate or corrupt the path portion of
/// the URI ('%' first — it is the escape introducer). Without this, a path
/// containing '?' or '#' would open a DIFFERENT file than the intended one.
/// Shared by both checkers so the two can never drift apart.
...
mod native_checks {
    ...
    use crate::database::EncryptedConnection;
    use crate::database::native::sqlcipher_connection::{
        pragma_key, pragma_plaintext_header, pragma_salt,
    };
    use diesel::connection::SimpleConnection;
    use xmtp_configuration::BUSY_TIMEOUT;

    /// Open a dedicated read-only checking connection and run the checks.
    /// `session_pragmas` is the SQLCipher session setup (key + plaintext
    /// header + salt) for encrypted databases, `None` for unencrypted.
    /// Never creates, migrates, or writes to the database.
    pub(crate) fn check_database_path(
        ...
        // `mode=rw` (no `mode=rwc`) omits SQLite's CREATE flag: if the file
        // vanishes between the `Path::exists()` check above and this
        // `establish` call, opening fails instead of silently recreating an
        // empty database that would then report a false `Ok`. Read-write
        // (not read-only) is still required so SQLite can perform WAL
        // recovery (replay/checkpoint the `-wal` file) on open.
        //
        // The URI escaping matters here too: without it a path containing
        // '?' or '#' would open a DIFFERENT file than the one
        // `Path::exists()` just validated.
    ...
    pub fn check_database_integrity(
        ...
        // Check the database file itself before touching the salt sidecar:
        // a deleted or mistyped path must report the missing DATABASE, not
        // the (necessarily also missing) sidecar — `SaltMissing` implies
        // the database exists but its sidecar is gone.
        if !std::path::Path::new(db_path).exists() {
            return IntegrityCheckResult::Failed {
                error: format!("database file does not exist: {db_path}"),
            };
        }
        let session_pragmas = match key {
            None => None,
            Some(key) => {
                let salt_path = match EncryptedConnection::salt_file(db_path) {
                    Ok(p) => p,
                    Err(e) => {
                        return IntegrityCheckResult::Failed {
                            error: format!("cannot resolve salt path: {e}"),
                        };
                    }
                };
                let salt_hex = match std::fs::read_to_string(&salt_path) {
                    ...
                    Err(e) => {
                        return IntegrityCheckResult::Failed {
                            error: format!("cannot read salt file: {e}"),
                        };
                    }
                };
                // The sidecar's contents are interpolated into a PRAGMA that
                // executes BEFORE `query_only = ON`, so treat the file as
                // untrusted input: a valid salt is exactly 16 bytes = 32 hex
                // chars (see `Salt` in sqlcipher_connection.rs). Anything
                // else — corruption or tampering — is rejected instead of
                // interpolated.
                if salt_hex.len() != 32 || !salt_hex.bytes().all(|b| b.is_ascii_hexdigit()) {
                    return IntegrityCheckResult::Failed {
                        error: "invalid SQLCipher salt sidecar: expected 32 hexadecimal characters"
                            .into(),
                    };
                }
                Some(format!(
                    ...
                    pragma_salt(salt_hex)
                ))
            }
        };
        check_database_path(db_path, session_pragmas.as_deref(), level)
    }
}
