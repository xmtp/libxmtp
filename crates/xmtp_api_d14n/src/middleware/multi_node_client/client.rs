...
impl<T: Client> Client for MultiNodeClient<T> {
    async fn request(
        ...
        inner
            .request(request, path, body).await
    }
    ...
    async fn stream(
        ...
        inner
            .stream(request, path, body).await
    }
}
...
mod tests {
    ...
    fn create_multinode_client_builder() -> MultiNodeClientBuilder<ClientBuilder> {
    ...
    async fn build_multinode_as_d14n() {
        ...
        multi_node_builder
            .gateway(gateway_builder.clone().build().unwrap())
        ...
        multi_node_builder
            .timeout(xmtp_common::time::Duration::from_millis(1000))
            .unwrap();
    ...
    async fn build_multinode_as_standalone() {
        ...
        let mut multi_node_builder = MultiNodeClient::builder();
        multi_node_builder
            .gateway_client(gateway_builder.clone().build().unwrap())
            .expect("gateway set on multi-node");
        ...
        multi_node_builder
            .timeout(xmtp_common::time::Duration::from_millis(100))
            .unwrap();
