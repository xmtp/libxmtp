...
//!     live lane the frame is **withheld** for it and flushed, in cursor
//!     order and watermark-deduped, right before its `CatchUpComplete` —
//!     delivered now it would jump ahead of the owed replay, and a later
//!     overlapping replay would re-serve or skip it;
    ...
    B::WelcomeMessage: Clone,
{
    ...
    /// cursor order, watermark-deduped, right before this lease's
    /// `CatchUpComplete`. They survive a reopen unscathed: the re-issue's
    /// replay either stays below them or re-delivers them through the owed
    /// lane, and the watermark skip at flush absorbs the overlap either
    /// way. Dropped with the lease.
    held_group: HashMap<Topic, Vec<B::GroupMessage>>,
    held_welcome: HashMap<Topic, Vec<B::WelcomeMessage>>,
    ...
    B::WelcomeMessage: Clone,
{
    ...
    fn complete(&mut self, lease: LeaseId, dropped: &mut Vec<LeaseId>) {
        ...
        let mut flush_group = Vec::new();
        let mut flush_welcome = Vec::new();
        if let Some(state) = self.leases.get_mut(&lease) {
            state.caught_up = true;
            // Withheld fresh frames join the feed now — cursor order within
            // each topic, watermark-deduped (a replay may have re-served
            // them through the owed lane in the meantime) — AHEAD of the
            // completion marker: they are the tail of history.
            for (topic, held) in std::mem::take(&mut state.held_group) {
                let replayed = state.replayed.get(&topic);
                flush_group.extend(held.into_iter().filter(|message| {
            ...
            for (topic, held) in std::mem::take(&mut state.held_welcome) {
                let replayed = state.replayed.get(&topic);
                flush_welcome.extend(held.into_iter().filter(|message| {
        ...
        if !flush_group.is_empty() && !self.deliver(lease, LeaseEvent::GroupMessages(flush_group)) {
            dropped.push(lease);
            return;
        }
        if !flush_welcome.is_empty()
            && !self.deliver(lease, LeaseEvent::WelcomeMessages(flush_welcome))
        {
            dropped.push(lease);
            return;
        }
    ...
    fn demux<M: Clone>(
        ...
        held_of: impl Fn(&mut LeaseState<B>) -> &mut HashMap<Topic, Vec<M>>,
        kind: DeliveryKind,
        ...
        let riders: (Option<LeaseId>, &[LeaseId]) = wave
            ...
            .unwrap_or((None, &[]));
        let riders = (riders.0, riders.1.to_vec());
