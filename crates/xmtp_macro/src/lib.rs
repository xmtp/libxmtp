...
///   `true`/`false`/`when`.
///
///   Under `#[retry(default = true)]`, a variant that absorbs a foreign error via
///   `#[from]` **must** carry its own `#[retry(...)]` — an unannotated one is a
///   compile error. A retryable-by-default enum inverts the usual failure mode: a
///   forgotten annotation there would retry a permanent error forever, and the
///   foreign error's classification is invisible at the `#[from]` site, so the
///   policy has to be stated (`true`, `false`, `inherit`, or `when`).
///
...
///
/// The generated impl always targets the trait's **default specialization**
/// (`RetryableError<NotSpecialized>`). Specialized impls such as
/// `RetryableError<Mls>` cannot be derived and stay hand-written; `inherit`
/// still forwards through an inner type whose only impl is specialized, since
/// method resolution picks the single applicable impl.
///
/// Variant attributes (mutually exclusive — combining any two, or repeating
