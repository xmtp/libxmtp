...
pub fn derive_retryable(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let body = match &input.data {
        Data::Enum(data_enum) => {
            ...
            let container_default = container.default;
            ...
            if data_enum.variants.is_empty() {
                ...
                let arms: Result<Vec<_>, syn::Error> = data_enum
                    .variants
                    .iter()
                    .map(|variant| variant_arm(variant, container_default))
                    .collect();
                match arms {
                    Ok(arms) => quote! {
                        match self {
...
///
/// `container_default` is the enum's `#[retry(default = <bool>)]` as written —
/// `None` when the container omitted it. The distinction matters: an *explicit*
/// `default = true` makes an unannotated `#[from]` variant a hard error (see
/// [`check_from_under_default_true`]), while the implicit `false` baseline does
/// not.
fn variant_arm(
    variant: &syn::Variant,
    ...
    let baseline = container_default.unwrap_or(false);
    let attr = RetryAttr::parse(&variant.attrs)?;
    check_from_under_default_true(variant, container_default, &attr)?;
...
/// Reject an unannotated `#[from]` variant on a `#[retry(default = true)]` enum.
///
/// A retryable-by-default enum inverts the usual failure mode: forgetting an
/// annotation on a variant that silently absorbs a *foreign* error (via
/// `#[from]`) means a permanent failure gets retried forever. The foreign
/// error's own classification is invisible at the `#[from]` site, so the author
/// has to state the policy rather than inherit a blanket `true`.
///
/// Only an explicit `default = true` triggers this — `default = false` and the
/// implicit baseline both fail closed, and structs have no `#[from]` variants.
fn check_from_under_default_true(
    variant: &syn::Variant,
    container_default: Option<bool>,
    attr: &RetryAttr,
) -> syn::Result<()> {
    let annotated = attr.span.is_some();
    let wraps_foreign = variant
        .fields
        .iter()
        .any(|field| field.attrs.iter().any(|a| a.path().is_ident("from")));

    if container_default == Some(true) && !annotated && wraps_foreign {
        return Err(syn::Error::new_spanned(
            &variant.ident,
            "variant wraps a foreign error via `#[from]` under `#[retry(default = true)]`; \
             a forgotten annotation would silently retry a permanent error — state the policy \
             explicitly with `#[retry(true)]`, `#[retry(false)]`, `#[retry(inherit)]`, or \
             `#[retry(when = ...)]`",
        ));
    }
    Ok(())
