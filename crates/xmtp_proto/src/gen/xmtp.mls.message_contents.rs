...
///   are unset. The enabling commit MUST also leave GROUP_MEMBERSHIP's
///   `ComponentMetadata.external_committer_permissions` admitting a
///   joiner writing its own entry (populating it in the same commit
///   when absent): every conforming external commit is structurally
///   required to write that entry, so without the grant the switch is
///   on but every join dead-ends at validation. The grant is an
///   ordinary permissions block — not a protocol-baked default — so
///   applications can evolve it later like any other permissions
///   surface.
///
