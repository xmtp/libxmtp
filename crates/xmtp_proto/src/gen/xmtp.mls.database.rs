...
pub mod app_data_update_data {
    /// One additional component write carried by the same commit as the
    /// primary V1 update. Same field semantics as V1's component_id /
    /// payload.
    #[derive(Clone, PartialEq, Eq, Hash, ::prost::Message)]
    pub struct Update {
        /// u16 component_id widened to u32 (proto has no u16).
        #[prost(uint32, tag = "1")]
        pub component_id: u32,
        /// Verbatim on-wire AppDataUpdate proposal payload.
        #[prost(bytes = "vec", tag = "2")]
    ...
    impl ::prost::Name for Update {
        ...
        const PACKAGE: &'static str = "xmtp.mls.database";
        fn full_name() -> ::prost::alloc::string::String {
            "xmtp.mls.database.AppDataUpdateData.Update".into()
        }
        fn type_url() -> ::prost::alloc::string::String {
            "/xmtp.mls.database.AppDataUpdateData.Update".into()
        }
    ...
    pub struct V1 {
        ...
        /// external_committer_permissions grant in the same commit
        /// (XIP-82 enable atomicity). Old readers ignore the field
        /// (additive), but the intents table is local-only so mixed
        /// versions never read each other's rows.
        #[prost(message, repeated, tag = "3")]
        pub additional_updates: ::prost::alloc::vec::Vec<Update>,
    }
    impl ::prost::Name for V1 {
        const NAME: &'static str = "V1";
        const PACKAGE: &'static str = "xmtp.mls.database";
