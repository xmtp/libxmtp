...
mod tests {
    ...
    async fn test_poll_next_successful_decode() {
        ...
        let inner_stream =
            BytesStream::new(stream::once(async move { Ok::<Bytes, ApiClientError>(Bytes::from(encoded_bytes)) }));
        let xmtp_stream =
    ...
    async fn test_poll_next_error_mapping() {
        let inner_stream = BytesStream::new(stream::once(async { Err::<Bytes, ApiClientError>(ApiClientError::WritesDisabled) }));
        let xmtp_stream =
            XmtpStream::<TestMessage>::new(inner_stream, ApiEndpoint::SubscribeGroupMessages);
        pin_mut!(xmtp_stream);
