...
in
{
  ...
  # WASM tooling
  wasmTools = [ wasm-bindgen-cli wasm-pack binaryen emscripten wasm-tools ];

  # Cargo workflow tools
  cargoTools = [ cargo-nextest cargo-deny cargo-machete cargo-hakari ];

  # CI-only cargo tools (coverage — Linux only)
  cargoCiTools = lib.optionals isLinux [ lcov cargo-llvm-cov ];

  # Protobuf tools
  protoTools = [ buf protobuf protolint ];

  # Lint tools
  lintTools = [ taplo shellcheck ];
  ...
  miscDevTools = [ jq curl graphite-cli toxiproxy omnix ];
}
