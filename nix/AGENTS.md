...
- Backend builds restore only the shared dependency sources over the dummy workspace. Include migrations and `.sqlx` metadata. Build with `SQLX_OFFLINE=true`; no database is allowed during the build.
