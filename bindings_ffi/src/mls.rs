...
mod tests {
    ...
    use xmtp_mls::{
        ...
        utils::{client_events::upload_debug_archive, PasskeyUser, Tester},
        InboxOwner,
    };
    ...
    async fn test_streams_and_parallel_messages() {
        ...
        let provider = Arc::new(caro.inner_client.mls_provider());
        let key = upload_debug_archive(&provider, "http://localhost:5559")
            .await
            .unwrap();
        tracing::info!("{key}");
        return;
