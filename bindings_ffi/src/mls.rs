...
mod tests {
    ...
    #[tokio::test]
    async fn test_massive_sync_and_consent_race() {
        use std::sync::Arc;
        use std::time::Instant;

        // Create primary and secondary clients
        let primary = Tester::new().await;
        let secondary = Tester::new().await;
        ...
        for peer_id in peer_ids {
            let _ = primary
                .conversations()
                .find_or_create_dm_by_inbox_id(peer_id.to_string(), FfiCreateDMOptions::default())
                .await
                .unwrap();
        }

        // Create groups with secondary
        let inbox_id = primary.inbox_id();
        for _ in 0..200 {
            let g = secondary
                .conversations()
                .create_group_with_inbox_ids(
                    (&[inbox_id.clone()]).to_vec(),
                    FfiCreateGroupOptions::default(),
                )
                .await
                .unwrap();
            for _ in 0..5 {
                g.send(vec![0]).await.unwrap();
            }
        }

        println!("Starting stream setup and initial list calls");

        // Start message stream and stream_all_messages concurrently
        let stream_cb = Arc::new(RustStreamCallback::default());
        let stream_cb2 = Arc::new(RustStreamCallback::default());
        let _stream_all = primary
            .conversations()
            .stream_all_messages(stream_cb.clone(), None)
            .await;

        let _stream = primary.conversations().stream(stream_cb2.clone()).await;

        // Pre-sync: list allowed and unknown
        let allowed_pre = primary
            .conversations()
            .list(FfiListConversationsOptions {
                consent_states: Some(vec![FfiConsentState::Allowed]),
                ..Default::default()
            })
            .unwrap();

        let unknown_pre = primary
            .conversations()
            .list(FfiListConversationsOptions {
                consent_states: Some(vec![FfiConsentState::Unknown]),
                ..Default::default()
            })
            .unwrap();

        println!(
            "Before sync: allowed = {}, unknown = {}",
            allowed_pre.len(),
            unknown_pre.len()
        );
        ...
        primary
            .conversations()
            .sync_all_conversations(Some(vec![FfiConsentState::Allowed]))
            .await
            .unwrap();
        let duration = start.elapsed();

        // Post-sync: list allowed and unknown
        let allowed_post = primary
            .conversations()
            .list(FfiListConversationsOptions {
                consent_states: Some(vec![FfiConsentState::Allowed]),
                ..Default::default()
        ...
        primary
            .conversations()
            .sync_all_conversations(Some(vec![FfiConsentState::Unknown]))
            .await
            .unwrap();
        let duration2 = start2.elapsed();

        let unknown_post = primary
            .conversations()
            .list(FfiListConversationsOptions {
                consent_states: Some(vec![FfiConsentState::Unknown]),
                ..Default::default()
        ...
        println!(
            "After sync: allowed = {}, unknown = {}, sync duration = {:?} sec",
            allowed_post.len(),
            unknown_post.len(),
        ...
        println!(
            "After sync unknown: allowed = {}, unknown = {}, sync duration = {:?} sec",
            allowed_post.len(),
            unknown_post.len(),
        ...
        assert!(
            duration.as_secs_f64() < 15.0,
            "Sync should complete in under 15 seconds"
        );

        assert!(
            duration2.as_secs_f64() < 15.0,
            "Sync should complete in under 15 seconds"
        );

        // Clean up streams
        _stream_all.end_and_wait().await.unwrap();
        _stream.end_and_wait().await.unwrap();
    }
}
