...
mod tests {
    ...
    async fn test_massive_sync_and_consent_race() {
        ...
        // Create 100+ groups with 1 other inbox (secondary)
        let inbox_id = secondary.inbox_id();
        for _ in 0..200 {
            let _ = primary
                .conversations()
                .create_group_with_inbox_ids((&[inbox_id.clone()]).to_vec(), FfiCreateGroupOptions::default())
                .await
                .unwrap();
        }

        println!("Starting sync + stream race");

        let start = Instant::now();

        // Start streamAllMessages
        ...
        // Run syncAllConversations concurrently
        let _ = primary
            .conversations()
            .sync_all_conversations(None)
            .await
        ...
        println!(
            "184 groups, 100+ DMs created; syncAll + stream finished in {:?} seconds",
            duration.as_secs_f64()
        );

        assert!(
            duration.as_secs_f64() < 15.0,
            "Sync and stream should complete within 15 seconds"
        );
