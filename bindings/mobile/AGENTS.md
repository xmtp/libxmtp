...
- `create_client` accepts optional `FfiClientRuntimeOptions`. Its `change_callbacks` and `stream_settings` fields default to `None`. Raw FFI callers group the old trailing arguments in this record; public SDK creation options stay unchanged. `FfiStreamSettings` limits are optional. Timer overrides use milliseconds. Conversion fills core defaults and uses core validation before the client is built.
