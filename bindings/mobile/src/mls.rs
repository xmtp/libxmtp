...
impl XmtpApiClient {
    fn inner(&self) -> ApiClientWrapper<xmtp_mls::XmtpApiClient> {
        self.wrapper.clone()
    }
}
...
impl FfiConversation {
    ...
    /// Edit a previously sent message. Returns the ID of the edit message.
    pub fn edit_message(
        &self,
        message_id: Vec<u8>,
        ...
        use prost::Message;
        use xmtp_proto::xmtp::mls::message_contents::EncodedContent;
        let edited_content = EncodedContent::decode(edited_content_bytes.as_slice())
            .map_err(|e| FfiError::generic(e.to_string()))?;
        let edit_id = self.inner.edit_message(message_id, edited_content)?;
        Ok(edit_id)
    }
