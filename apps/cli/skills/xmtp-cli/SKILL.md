...
```bash
# generate keys and save backend configuration to ~/.xmtp/.env
xmtp init --backend-url http://127.0.0.1:5050

# output keys to console instead of writing to a file
xmtp init --backend-url http://127.0.0.1:5050 --stdout

# generate keys with a custom database label and output path
xmtp init --backend-url https://backend.example.com --env staging-a --output ./my-project/.env

# overwrite existing config file
xmtp init --backend-url http://127.0.0.1:5050 --force
```
...
- `XMTP_BACKEND_URL` - Required backend URL
- `XMTP_ENV` - Database label only (default: `local`)
...
Set `XMTP_BACKEND_URL` or pass `--backend-url` to every command that creates a
client or contacts the backend. There is no built-in backend default.
`XMTP_ENV` is only a database label and never selects a backend.
...
```bash
# 1. initialize with a backend URL (first time only)
xmtp init --backend-url http://127.0.0.1:5050
...
5. **Specify backend**: Save `XMTP_BACKEND_URL` with `xmtp init` or pass `--backend-url`
