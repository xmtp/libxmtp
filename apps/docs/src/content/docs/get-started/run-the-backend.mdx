...
| `resource_attributes` | map of strings                 | `{}`           | Extra resource attributes. `service.name` and `service.version` are reserved.                      |
...
Omit this section to disable caller authentication. To enable caller authentication,
set at least one mechanism: `api_keys`, `jwks_url`, or a non-empty `keys` array.
Do not set both `jwks_url` and `keys`. An empty
`[auth]` section fails startup. Requests then require a valid bearer token,
except for gRPC health and CORS preflight. Streams are checked when they open.
