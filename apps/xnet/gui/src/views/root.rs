...
use gpui::{
    AsyncApp, Context, ScrollHandle, Timer, WeakEntity, Window, deferred, div, hsla, prelude::*,
    px,
};
...
impl RootView {
    ...
    fn render_toxics_body(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if xmtpd_names.is_empty() {
            ...
            for name in &xmtpd_names {
                nodes_panel =
                    nodes_panel.child(self.render_proxy_latency_controls(name, cx));
            }
        }
        ...
        if infra_names.is_empty() {
            ...
            for name in &infra_names {
                infra_panel =
                    infra_panel.child(self.render_proxy_latency_controls(name, cx));
            }
        }
    ...
    fn render_proxy_latency_controls(
        ...
        let presets: &[(u32, &str)] =
            &[(50, "50ms"), (100, "100ms"), (250, "250ms"), (500, "500ms"), (1000, "1s")];
