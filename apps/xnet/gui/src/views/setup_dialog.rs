...
use gpui::{div, prelude::*, px, ClipboardItem, SharedString};
...
fn render_platform_dns_instructions(container: gpui::Div) -> gpui::Div {
    container.child(
        div()
            .text_color(theme::text_secondary())
            .text_xs()
            .child("Configure your system DNS to resolve *.xmtpd.local to 127.0.0.1 via port 5354."),
    )
}
...
fn code_block(text: &str) -> impl IntoElement {
    ...
    div()
        ...
        .child(
            div()
                .id(SharedString::from(format!("copy-{}", &text_for_clipboard[..text_for_clipboard.len().min(32)])))
                .cursor_pointer()
                .px(px(6.0))
                .py(px(4.0))
                .rounded(px(4.0))
