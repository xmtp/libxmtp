...
use crate::test_support::{TestServer, TestResult};
...
impl Native {
    pub async fn open(server: &TestServer) -> TestResult<Self> {
        ...
        let output = api::subscription_service_client::SubscriptionServiceClient::new(server.channel.clone())
            .subscribe(ReceiverStream::new(receiver)).await?.into_inner();
        let mut stream = Self { input, output };
        assert!(matches!(stream.next().await?, Frame::Started(_)));
        Ok(stream)
    }
    pub async fn send(&self, request: Input) -> TestResult { self.input.send(api::SubscribeRequest { request: Some(request) }).await?; Ok(()) }
    pub async fn next(&mut self) -> TestResult<Frame> {
        Ok(xmtp_common::time::timeout(xmtp_common::time::Duration::from_secs(5), self.output.message()).await??.ok_or("stream ended")?.response.ok_or("empty response")?)
    }
    pub async fn update(&self, id: u64, adds: Vec<api::TopicQuery>, removes: Vec<api::Topic>) -> TestResult {
        self.send(Input::Update(api::subscribe_request::Update { id, adds, removes })).await
    }
    pub async fn messages(&mut self, count: usize) -> TestResult<Vec<api::ServerEnvelope>> {
        ...
        while rows.len() < count {
            match self.next().await? {
                ...
                Frame::Ping(ping) => self.send(Input::Pong(api::Pong { nonce: ping.nonce })).await?,
...
pub fn envelope(topic: u8, value: u8) -> api::ClientEnvelope {
    ...
    if let Some(api::client_envelope::Payload::WelcomeMessage(welcome)) = &mut envelope.payload
        && let Some(api::welcome_message::Version::V1(welcome)) = &mut welcome.version { welcome.data.push(value); }
    envelope
}
