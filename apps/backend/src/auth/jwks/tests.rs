...
async fn stale_keys_use_the_server_drain_and_return_a_typed_error() {
    ...
    // A unary call still serves with the refreshed key set. The Get RPC was
    // removed upstream, so this queries an empty topic instead.
    let mut request = tonic::Request::new(api::QueryRequest {
        queries: vec![query_topic(
            topic(xmtp_proto::types::TopicKind::WelcomeMessagesV1, &[92; 32]),
            0,
        )],
        limit: 1,
    });
