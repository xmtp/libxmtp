...
async fn bootstrap(
    ...
    let (boundary, last_attempt) = loop {
        ...
        if let Some(boundary) = db::boundary::advance(primary, wait).await? {
            break (boundary, attempt);
        }
        sleep(interval).await;
    };
...
async fn run(
    ...
    loop {
        let (next_forward, next_gaps, rows, immediate, boundary) =
            poll(&mut connection, forward, &gaps)
                .await
                .map_err(|_| Status::unavailable("tailer database read failed"))?;
        if next_gaps.len() > max_gaps {
            return Err(Status::resource_exhausted("tailer gap capacity exceeded"));
        }
        registry.dispatch(rows);
        forward = next_forward;
        gaps = next_gaps;
        if gaps.iter().any(|&(_, high)| high > boundary) {
...
impl PollPage {
    fn select(&mut self, candidate: Candidate) -> bool {
        ...
        if self.bytes + size > FETCH_BUFFER_BYTES {
            return false;
        }
        self.bytes += size;
...
async fn poll(
    ...
    'gaps: for batch in gaps.chunks(GAP_BATCH) {
        ...
        for &(low, high) in batch {
            let candidates: Vec<_> = candidates
                .iter()
                .filter(|candidate| candidate.sequence_id >= low && candidate.sequence_id <= high)
                .collect();
            let mut consumed = true;
            for candidate in &candidates {
                if !page.select((*candidate).clone()) {
                    consumed = false;
                    complete = false;
                    immediate = true;
                    break;
        ...
        if page.bytes >= FETCH_BUFFER_BYTES.saturating_sub(ENVELOPE_OVERHEAD) {
            complete = false;
            immediate = true;
            break;
        }
    ...
    if complete {
        ...
        for candidate in candidates {
            ...
            if !page.select(candidate) {
                immediate = true;
                break;
            }
            if id > next_forward + 1 {
                page.gaps.insert(next_forward + 1, id - 1);
            }
            next_forward = id;
        }
    }
    let ids: Vec<_> = page
        .selected
        .iter()
        .map(|candidate| candidate.sequence_id)
        .collect();
    let rows = if ids.is_empty() {
        Vec::new()
    } else {
        db::stream::payloads(&mut tx, &ids).await?
    };
    if rows.len() != ids.len() {
...
fn subtract(ranges: &mut BTreeMap<i64, i64>, low: i64, high: i64) {
    ...
    let first = ranges
        .range(..=low)
        .next_back()
        .map_or(low, |(&start, _)| start);
    let overlaps: Vec<_> = ranges
        .range(first..=high)
        .filter(|(_, end)| **end >= low)
        .map(|(&start, &end)| (start, end))
        .collect();
    for (start, end) in overlaps {
        ranges.remove(&start);
        if start < low {
            ranges.insert(start, low - 1);
        }
        if end > high {
            ranges.insert(high + 1, end);
        }
    }
}
