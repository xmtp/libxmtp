...
impl Default for Mailbox {
    fn default() -> Self {
        Self {
            ...
            frame_bytes: DELIVERY_FRAME_BYTES,
        }
    }
}
struct Watcher {
...
impl Registry {
    ...
    pub(super) fn connect(&self, mailbox: Arc<Mailbox>, kind: StreamKind) -> Result<u64, Status> {
        ...
        state.next_id = state
            .next_id
            .checked_add(1)
            .ok_or_else(|| Status::resource_exhausted("session identifiers exhausted"))?;
    ...
    pub(super) fn current(&self, id: u64, topic: &[u8], floor: i64) -> i64 {
        ...
        if needed <= floor
            && let Some(watcher) = state
                .topics
                .get_mut(topic)
                .and_then(|watchers| watchers.get_mut(&id))
        {
    ...
    pub(super) fn dispatch(&self, rows: Vec<StoredEnvelope>) {
        ...
        for row in rows {
            let row = Arc::new(row);
            if let Some(watchers) = state.topics.get(&row.topic) {
                for (&id, watcher) in watchers {
                    if watcher.current {
                        ...
                        let head = mail
                            ...
                            .or_insert((watcher.generation, row.sequence_id));
...
fn enqueue(mailbox: &Mailbox, rows: Vec<(u64, Arc<StoredEnvelope>)>) {
    ...
    for row in rows {
        ...
        if bytes + size > mailbox.frame_bytes && !pending.is_empty() {
            flush(mailbox, std::mem::take(&mut pending), bytes);
            bytes = 0;
        }
        bytes += size;
        pending.push(row);
