use super::{ENVELOPE_OVERHEAD, FETCH_ROWS, StreamHub, output::Reservation};
...
pub(super) async fn fetch(
    ...
    let candidates = db::stream::history(&mut tx, &ranges, FETCH_ROWS)
        .await
        .map_err(|_| Status::unavailable("history read failed"))?;
    let mut bytes = 0;
    let mut selected = HashMap::new();
    let mut visited = requests.len();
    for (ordinal, candidate) in candidates {
        if candidate.topic != requests[ordinal].range.topic {
            return Err(Status::unavailable("history topic mismatch"));
        }
        let size = candidate.payload_bytes as usize + ENVELOPE_OVERHEAD;
        if bytes + size > budget {
            if selected.is_empty() {
                return Err(Status::resource_exhausted(
                    "stored envelope exceeds delivery capacity",
                ));
            }
            visited = ordinal;
            if selected.values().any(|&value| value == ordinal) {
                visited += 1;
            }
            break;
        }
    ...
    for ordinal in 0..visited {
        if !selected.values().any(|&value| value == ordinal) {
            return Err(Status::unavailable(
                "visible topic head has no retained envelope",
            ));
        }
    }
    let rows = db::stream::payloads(&mut tx, &ids)
        .await
        .map_err(|_| Status::unavailable("history payload read failed"))?;
    if rows.len() != ids.len() {
        return Err(Status::unavailable("history candidate disappeared"));
    }
    tx.commit()
        .await
