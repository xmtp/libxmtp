...
use super::{GLOBAL_LOCK_DOMAIN, ALLOCATION_BARRIER};
...
pub(crate) async fn advance(pool: &PgPool, wait_ms: u64) -> Result<Option<i64>, Error> {
    let mut tx = pool.begin_with("BEGIN ISOLATION LEVEL READ COMMITTED").await?;
    let wait = format!("{wait_ms}ms");
    sqlx::query!("SELECT set_config('lock_timeout', $1, true)", wait).fetch_one(&mut *tx).await?;
    if let Err(error) = sqlx::query!("SELECT pg_advisory_xact_lock($1::integer, $2::integer)", GLOBAL_LOCK_DOMAIN, ALLOCATION_BARRIER).execute(&mut *tx).await {
    ...
    if updated != Some(boundary) { return Err(Error::Invariant("missing or rewound allocation boundary")); }
    tx.commit().await?;
    Ok(Some(boundary))
}
