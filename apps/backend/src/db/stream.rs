...
pub(crate) async fn snapshot(pool: &PgPool) -> Result<Transaction<'static, Postgres>, Error> {
    Ok(pool.begin_with("BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY").await?)
}
...
pub(crate) async fn snapshot_connection(connection: &mut PgConnection) -> Result<Transaction<'_, Postgres>, Error> {
    Ok(connection.begin_with("BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY").await?)
}
...
pub(crate) async fn boundary(tx: &mut Transaction<'_, Postgres>) -> Result<i64, Error> {
    sqlx::query_scalar!("SELECT closed_sequence_id FROM allocation_boundary WHERE singleton")
        .fetch_optional(&mut **tx).await?.ok_or(Error::Invariant("allocation boundary missing"))
}
...
pub(crate) async fn heads(pool: &PgPool, topics: &[Vec<u8>]) -> Result<Vec<i64>, Error> {
    Ok(sqlx::query!(r#"SELECT COALESCE(w.last_sequence_id, 0) AS "head!"
        FROM unnest($1::bytea[]) WITH ORDINALITY AS wanted(topic, ordinal)
        LEFT JOIN topic_watermark w ON w.topic = wanted.topic ORDER BY wanted.ordinal"#, topics)
        .fetch_all(pool).await?.into_iter().map(|row| row.head).collect())
}

/// Select bounded forward candidates without using the closed boundary as a ceiling.
pub(crate) async fn forward(tx: &mut Transaction<'_, Postgres>, after: i64, limit: i64) -> Result<Vec<Candidate>, Error> {
    Ok(sqlx::query_as!(Candidate, r#"SELECT sequence_id, topic, octet_length(payload) AS "payload_bytes!"
        FROM envelopes WHERE sequence_id > $1 ORDER BY sequence_id LIMIT $2"#, after, limit)
        .fetch_all(&mut **tx).await?)
}

/// Probe bounded missing ranges in sequence order. A full page is not exhaustion.
pub(crate) async fn gaps(tx: &mut Transaction<'_, Postgres>, ranges: &[(i64, i64)], limit: i64) -> Result<Vec<Candidate>, Error> {
    let (low, high): (Vec<_>, Vec<_>) = ranges.iter().copied().unzip();
    Ok(sqlx::query_as!(Candidate, r#"SELECT e.sequence_id, e.topic, octet_length(e.payload) AS "payload_bytes!"
        ...
        ORDER BY e.sequence_id"#, &low, &high, limit).fetch_all(&mut **tx).await?)
}

/// Probe topics in caller-supplied fair order. Return sizes before allocating payloads.
pub(crate) async fn history(tx: &mut Transaction<'_, Postgres>, ranges: &[Range], limit: i64) -> Result<Vec<(usize, Candidate)>, Error> {
...
pub(crate) async fn payloads(tx: &mut Transaction<'_, Postgres>, ids: &[i64]) -> Result<Vec<StoredEnvelope>, Error> {
