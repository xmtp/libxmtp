...
data class ReactionV2Codec(
    ...
    override fun encode(content: FfiReactionPayload): EncodedContent = EncodedContent.parseFrom(encodeReaction(content))

    override fun decode(content: EncodedContent): FfiReactionPayload = decodeReaction(content.toByteArray())
    ...
    override fun shouldPush(content: FfiReactionPayload): Boolean =
        when (content.action) {
