...
impl GroupMessage {
    ...
    pub fn timestamp(&self) -> i64 {
        self.created_ns.timestamp_nanos_opt().expect("timestamp out of range for i64, are we in 2262 A.D?")
    }
...
mod test {
    use chrono::DateTime;
    use openmls::prelude::PublicMessage;
    use xmtp_common::{rand_vec, Generate};

    use super::*;

    impl Generate for GroupMessage {
        fn generate() -> Self {
            GroupMessage {
                cursor: Default::default(),
                created_ns: DateTime::from_timestamp_nanos(xmtp_common::rand_i64()).into(),
                group_id: GroupId::generate(),
                message: PublicMessage::generate().into(),
                sender_hmac: rand_vec::<2>(),
                should_push: true,
                payload_hash: rand_vec::<32>(),
            }
        }
    }
}
