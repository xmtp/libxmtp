...
class Entry:
    ...
    def display_name(self) -> str:
        ...
        if self.sdk in {"Node", "Browser"} and (
            self.name
            in {
                "encryptAttachment",
                "decryptAttachment",
...
def swift_scan(text: str, source: str) -> list[Entry]:
    ...
    for number, line in enumerate(text.splitlines(), 1):
        ...
        explicit = re.match(
            r"^(?:@[_\w.]+(?:\([^)]*\))?\s+)*(?:public|open)\s+", stripped
        )
        parsed = compact_name(stripped, "Swift") if explicit else None
        scoped_extension = (
            re.match(r"^extension\s+([A-Za-z_]\w*)\b", stripped)
            if not explicit
            else None
        )
        if explicit and parsed and at_surface:
            ...
            if inherited_kind in {"extension", "protocol"}:
                ...
                if member and not stripped.startswith(
...
def swift_inventory() -> list[Entry]:
    ...
    entries.append(
        Entry(
            "Swift",
            "sdks/ios/Sources/XMTPiOS/Proto/*.pb.swift",
            0,
            "pattern: Proto/*.pb.swift public declarations",
            "generated family",
            proto_count,
        )
    )
    ...
    for entry in entries:
        ...
        for line in lines[entry.line - 1 : entry.line + 11]:
            ...
            if "{" in line or "=" in line or ";" in line:
                break
        directly_used.update(
            re.findall(
                r"\b(?:Ffi[A-Za-z0-9_]+|XmtpApiClient|DbOptions)\b",
                "\n".join(signature),
            )
        )
    patterns = [
        (
            "FfiConverter internals",
            re.compile(r"^(?:FfiConverter[^.]*|func FfiConverter[^ ]*)(?:\..*)?$"),
        ),
        (
            "callback protocols and implementations",
            re.compile(
                r"^Ffi(?!Converter)[A-Za-z0-9_]*(?:Callback|Listener)(?:Impl)?(?:\..*)?$"
            ),
        ),
        (
            "other Ffi binding types and members",
            re.compile(r"^Ffi[A-Za-z0-9_]+(?:\..*)?$"),
        ),
...
def kotlin_scan(text: str, source: str) -> list[Entry]:
    ...
    for number, line in enumerate(text.splitlines(), 1):
        ...
        at_surface = (
            (depth == 0)
            if not contexts
            else (contexts[-1][1] in {"type", "enum"} and depth == contexts[-1][0])
        )
        match = (
            KOTLIN_DECL.search(stripped)
        ...
        hidden = bool(
            match
            and re.search(
                r"\b(private|internal|protected)\b", stripped[: match.start()]
            )
        )
        if (
            match
            and owner_public
            and not hidden
            and (at_surface or class_header)
            and (not class_header or (pending is not None and pending[1]))
        ):
            kind, name = match.groups()
            name = name or (
                "Companion" if kind == "object" and "companion" in stripped else kind
            )
            if kind == "fun":
            ...
            if kind in {"val", "var"}:
                prop = re.search(
                    r"\b(?:val|var)\s+([A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*)", stripped
                )
                if prop:
                    name = prop.group(1)
            ...
            if kind == "class":
                for prop in re.finditer(
                    r"\b(?:val|var)\s+([A-Za-z_]\w*)", stripped[match.end() :]
                ):
                    entries.append(
                        Entry(
                            "Kotlin",
                            source,
                            number,
                            f"{symbol}.{prop.group(1)}",
                            "constructor property",
                        )
                    )
        elif (
            contexts
            and contexts[-1][1] == "enum"
            and depth == contexts[-1][0]
            and owner_public
        ):
            ...
            if variant:
                entries.append(
                    Entry(
                        ...
                        source,
                        number,
                        f"{owner}.{variant.group(1)}",
                        "enum case",
                    )
                )
        if at_surface and match and match.group(1) in {"class", "interface", "object"}:
            name = match.group(2) or (
                "Companion" if "companion" in stripped else match.group(1)
            )
            symbol = f"{owner}.{name}" if owner else name
            pending = (
                "enum" if "enum class" in stripped else "type",
                owner_public and not hidden,
                symbol,
            )
...
def kotlin_inventory() -> list[Entry]:
    return [
        ...
        for entry in kotlin_scan(path.read_text(), rel(path))
    ]
...
def ts_exports(path: Path, seen: set[Path]) -> list[Entry]:
        ...
        if names:
            for item in names.split(","):
                ...
                if target and re.search(
                    rf"export\s+(?:const|function)\s+{re.escape(name)}\b[\s\S]{{0,200}}?=>",
                    target.read_text(),
                ):
                    kind = "free function"
                entries.append(
                    Entry(sdk, rel(path) if target else spec, line, name, kind)
                )
                if target and re.search(
    ...
    for number, line in enumerate(text.splitlines(), 1):
        found = re.match(
            r"^export\s+(?:(?:declare|abstract|default)\s+)*(type|interface|class|enum|function|const|let|var)\s+([A-Za-z_]\w*)",
            line,
        )
        if found:
            kind, name = found.groups()
            if kind == "const" and re.search(
...
def ts_object_members(path: Path, type_name: str, sdk: str) -> list[Entry]:
    ...
    start = next(
        (
            ...
            if re.match(
                rf"^export\s+(?:type|interface)\s+{re.escape(type_name)}\b", line
            )
        ),
        None,
    )
...
def ts_class_members(path: Path, class_name: str, sdk: str) -> list[Entry]:
    ...
    for index in range(start, len(lines)):
        ...
        if (
            sdk == "Node"
            and class_name == "Conversation"
            and pending_signature
            and re.match(r"_client\s*:", stripped)
        ):
            entries.append(
                Entry(
                    sdk,
        ...
        if (
            entered
            and depth == 1
            and not pending_signature
            and stripped
            and not stripped.startswith(("//", "*", "#", "private ", "protected "))
        ):
            if re.match(r"\[Symbol\.asyncIterator\]\s*\(", stripped):
                entries.append(
                    Entry(
                        ...
                        rel(path),
                        index + 1,
                        f"{class_name}[Symbol.asyncIterator]",
                        "member",
                    )
                )
            else:
                match = re.match(
                    r"(?:(?:public|static|async|readonly|override|declare|get|set)\s+)*([A-Za-z_]\w*)\s*(?:[<(=:?]|$)",
                    stripped,
                )
...
def build() -> str:
    ...
    if open_items:
        lines += [
            f"{len(open_items)} exports need a design decision. Their proposed status appears in the SDK table.",
            "",
        ]
        lines += open_items
    else:
...
def self_test() -> None:
    ...
    assert {entry.name for entry in swift} >= {
        "UnstableGroup",
        "UnstableGroup.enableProposals",
        "Group.unstable",
    }
    kotlin = kotlin_scan(
    ...
    assert {
        ...
        "DecodedMessageV2",
        "DecodedMessageV2.contentTypeId",
        "MessageReader",
        "MessageReader.next",
        "NotificationError",
    ...
    with tempfile.TemporaryDirectory(dir=ROOT) as directory:
        ...
        assert "MessageStream[Symbol.asyncIterator]" in {
        ...
        assert names == {"Options.retry", "Options.nested", "Options.nested.enabled"}, (
            names
        )
    ...
    expected = {
        ...
        "Kotlin": {
            ...
            "DecodedMessage.SortBy.SENT_TIME": (
                "generated",
                "ListMessagesOptions.sortBy.sentAt",
            ),
        ...
        "Browser": {
            ...
            "Client.unsafe_applySignatureRequest": (
                "generated",
    ...
    for sdk, names in expected_open.items():
        by_name = {entry.name: entry for entry in inventories[sdk]}
        assert all(
            name in by_name and classify(by_name[name]).open for name in names
        ), (sdk, names)
    generated_source = SWIFT / "Libxmtp/xmtpv3.swift"
    actual_generated = len(
        swift_scan(generated_source.read_text(), rel(generated_source))
    )
    covered_generated = sum(
        entry.count
        for entry in inventories["Swift"]
        if entry.source == rel(generated_source)
    )
