...
ci-failures job:
    ...
    gh api $esc repos/xmtp/libxmtp/actions/jobs/{{ job }}/logs \
      ...
      | { rg -N '(^##\[error\]|^\s*FAIL |AssertionError|^thread .* panicked|^\s*assertion.*failed|^error(\[[^]]+\])?:|Tests\s+[0-9]+ failed|test result: FAILED|^\s*\S+ FAILED\s*$|^/.*\.(c|m)?[jt]sx?$|^\s*[0-9]+:[0-9]+\s+(error|warning)\s|^\s*[×✖]\s|^[[:space:]│╭╰─]*[^ ]+\.(c|m)?[jt]sx?:[0-9]+|^\[warn\] |Code style issues found)' || test $? -eq 1; } \
      | sort -u | sed -n '1,40p'
