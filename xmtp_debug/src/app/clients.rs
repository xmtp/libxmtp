...
pub async fn temp_client(
    ...
    let public = hex::encode(local_wallet.get_public_identifier());
...
async fn new_client_inner(
    ...
    let nonce = 1;
    let inbox_id = generate_inbox_id(&wallet.get_public_identifier(), &nonce)?;
...
pub async fn register_client(client: &crate::DbgClient, owner: impl InboxOwner) -> Result<()> {
    ...
    trace!(
        ...
        address = owner.get_public_identifier(),
    ...
    if let Some(mut req) = signature_request {
        ...
        warn!(address = owner.get_public_identifier(), "Signature request empty!");
    }
