...
impl Validator for NoMissingMessages {
    ...
    async fn validate(&self, ctx: &mut HealthContext) -> Vec<OpResult> {
        ...
        // Load everything once — per-group calls would rescan the whole
        // network per iteration.
        let by_group = ctx.recorded_messages_by_group();

        // Collect every (client, group, active_mls_group, expected) pair.
        // Inactive clients (left/removed) aren't held to convergence.
        let mut pairs = Vec::new();
        for group_id in ctx.existing_groups.iter().chain(ctx.new_groups.iter()) {
            let Ok(id_bytes) = <[u8; 16]>::try_from(group_id.as_slice()) else {
                continue;
            };
            let Some(expected) = by_group.get(&id_bytes) else {
                continue;
            };
            ...
            for client in &clients {
                let Ok(mls_group) = client.group(&group_id.to_vec()) else {
                    continue;
                };
        ...
        for (client, group_id, _mls_group, expected) in &pairs {
            ...
            let join_at_ns = match db.find_group(group_id.as_slice()) {
