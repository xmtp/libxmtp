...
//! the newly-created group via `MlsGroup::remove_members_by_inbox_id`.
//! The transient victim is created inside this op so nothing persisted
//! gets disturbed (avoids the cross-run "removed-and-stays-around"
//! fork-noise pattern).
//!
...
impl HealthOp for RemoveMember {
    ...
    async fn execute(&self, ctx: &mut HealthContext) -> Vec<OpResult> {
        ...
        let outcome: color_eyre::eyre::Result<()> = async {
            ...
            // `remove_members_by_inbox_id`. Both commits go through primary.
            let primary_group = ctx.primary.group(&gid.to_vec()).map_err(|e| eyre!("{e}"))?;
            primary_group
                .add_members_by_inbox_id(std::slice::from_ref(&victim_inbox))
                .await
                .map_err(|e| eyre!("{e}"))?;
            primary_group
