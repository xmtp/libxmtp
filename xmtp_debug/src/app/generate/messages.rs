...
impl GenerateMessages {
    ...
    pub async fn run(self, n: usize) -> Result<Vec<Duration>> {
        ...
        // Read XDBG_LOOP_PAUSE once for loop-mode pausing.
        ...
        if r#loop {
            loop {
                ...
                if let Some(secs) = loop_pause_secs {
                    tracing::debug!(secs, "sleeping XDBG_LOOP_PAUSE in message loop");
    ...
    async fn send_many_messages(&self, n: usize) -> Result<Vec<Duration>> {
        ...
        let mut set: tokio::task::JoinSet<Result<Duration, eyre::Error>> = tokio::task::JoinSet::new();
        ...
        for _ in 0..n {
            ...
            set.spawn(async move {
                let _permit = semaphore.acquire().await?;
                let latency = Self::send_message(&group, cs, n, opts).await?;
                bar_pointer.inc(1);
                Ok(latency)
            });
        }
        ...
        let latencies: Vec<Duration> = res
            .into_iter()
            .filter_map(|r| r.ok())
            .collect();
        Ok(latencies)
    ...
    async fn send_message(
        ...
        if let Some(inbox_id) = group.members.choose(rng) {
            // Acquire per-identity lock before touching the sender's MLS state.
            let id_lock = get_identity_lock(inbox_id)?;
