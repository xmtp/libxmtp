...
async fn test_inserted_after_filter() {
    with_connection(|conn| {
        ...
        let after_args = MsgQueryArgs {
            inserted_after: Some(second_inserted_at),
            ..Default::default()
        };
...
async fn test_inserted_before_filter() {
    with_connection(|conn| {
        ...
        let before_args = MsgQueryArgs {
            inserted_before: Some(fourth_inserted_at),
            ..Default::default()
        };
...
async fn test_inserted_at_based_pagination() {
    with_connection(|conn| {
        ...
        let page2_args = MsgQueryArgs {
            inserted_after: Some(last_inserted_page1),
            limit: Some(3),
            ..Default::default()
        };
        ...
        let page3_args = MsgQueryArgs {
            inserted_after: Some(last_inserted_page2),
            limit: Some(3),
            ..Default::default()
        };
...
#[xmtp_common::test]
async fn test_rowid_populated_in_all_queries() {
    with_connection(|conn| {
        let group = generate_group(None);
        group.store(conn).unwrap();
