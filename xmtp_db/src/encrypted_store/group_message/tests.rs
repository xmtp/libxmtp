...
#[xmtp_common::test]
fn test_get_latest_sequence_id_for_group() {
    let db = setup_test_db(); // or however you instantiate your test DB

    let group_id = b"test_group".to_vec();
    let msg1 = StoredGroupMessage {
        id: b"msg1".to_vec(),
    ...
    let msg3 = StoredGroupMessage {
        id: b"msg3".to_vec(),
        group_id: group_id.clone(),
        sequence_id: None, // should be ignored
        sent_at_ns: 300,
        ..default_msg_fields()
    };

    db.store(&msg1).unwrap();
    db.store(&msg2).unwrap();
    db.store(&msg3).unwrap();

    let latest = db.get_latest_sequence_id_for_group(&group_id).unwrap();
