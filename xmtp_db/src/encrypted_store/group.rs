...
#[builder(setter(into), build_fn(error = "StorageError", validate = "Self::validate"))]
#[derive(AsChangeset)]
/// A Unique group chat
...
impl StoredGroup {
    pub fn cursor(&self) -> Option<Cursor> {
        ...
        if let Some(sequence_id) = self.sequence_id && let Some(originator) = self.originator_id {
            return Some(Cursor {
            ...
            Ok(dsl::groups
                ...
                .map(|(seq, orig)| {
                    Cursor {
                        ...
                        originator_id: orig.expect("if seq is not null, originator must not be null") as u32
                    }
                })
                .collect())
        })
...
pub(crate) mod tests {
    ...
    async fn test_get_sequence_ids() {
        with_connection(|conn| {
            ...
            assert_eq!(vec![30, 10], conn.group_cursors().unwrap().into_iter().map(|c| c.sequence_id).collect::<Vec<u64>>());
        })
        .await
    }
