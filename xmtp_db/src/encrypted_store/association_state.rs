...
impl<R> QueryAssociationStateCache for &R where R: QueryAssociationStateCache {
    fn write_to_cache(
        &self,
        inbox_id: String,
        sequence_id: i64,
...
pub(crate) mod tests {
    ...
    async fn test_batch_read() {
        with_connection(|conn| {
            ...
            conn.write_to_cache(mock.inbox_id.clone(), 1, mock.clone()).unwrap();
            ...
            conn.write_to_cache(mock_2.inbox_id.clone(), 2, mock_2.clone()).unwrap();

            let first_association_state: Vec<MockState> =
                conn.batch_read_from_cache(vec![(mock.inbox_id.to_string(), 1)]).unwrap().into_iter().map(|i| i.try_into().unwrap()).collect();
            ...
            let both_association_states: Vec<MockState> =
                conn.batch_read_from_cache(vec![(mock.inbox_id.clone(), 1), (mock_2.inbox_id.clone(), 2)]).unwrap().into_iter().map(|i| i.try_into().unwrap()).collect();

            assert_eq!(both_association_states.len(), 2);

            let no_results = conn.batch_read_from_cache(vec![(mock.inbox_id.clone(), 2)]).unwrap().into_iter().map(|i| i.try_into().unwrap()).collect::<Vec<MockState>>();
