...
use crate::{StorageError, StoreOrIgnore, impl_store, impl_store_or_ignore};
...
impl<C: ConnectionExt> QueryRefreshState for DbConnection<C> {
    ...
    fn update_cursor<Id: AsRef<[u8]>>(
        ...
        let num_updated = self.raw_query_write(|conn| {
            let incomplete = diesel::insert_into(dsl::refresh_state)
                .values(&state)
                .on_conflict((dsl::entity_id, dsl::entity_kind, dsl::originator_id))
                .do_update()
                .set(dsl::cursor.eq(excluded(dsl::cursor)));

            query_dsl::methods::FilterDsl::filter(
                incomplete,
                dsl::cursor.lt(cursor.sequence_id as i64),
            )
            .execute(conn)
        })?;
        Ok(num_updated >= 1)
