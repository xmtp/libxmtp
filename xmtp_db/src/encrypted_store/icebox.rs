...
mod tests {
    ...
    fn iced(group_id: Vec<u8>) -> Vec<OrphanedEnvelope> {
        vec![
            ...
            OrphanedEnvelope::builder()
                ...
                .depending_on(Cursor::new(28, 2u32))
                .payload(vec![1, 2, 3])
                .group_id(group_id)
                .build()
                .unwrap(),
    ...
    fn icebox_dependency_chain() {
        with_connection(|conn| {
            ...
            let dep_chain = conn.past_dependants(&[Cursor::new(41, 1u32)])?;
            ...
            let mut dep_chain = conn.future_dependants(&[Cursor::new(39, 2u32)])?;
    ...
    fn test_icebox_wrong_originator() {
        with_connection(|conn| {
            ...
            let mut dep_chain = conn.past_dependants(&[Cursor::new(41, 1u32)])?;
            dep_chain.sort_by_key(|d| d.cursor.sequence_id);
            ...
            let dep_chain = conn.future_dependants(&[Cursor::new(39, 1u32)])?;
    ...
    fn test_icebox_wrong_sequence() {
        with_connection(|conn| {
            ...
            let dep_chain = conn.future_dependants(&[Cursor::new(100, 2u32)])?;
            assert_eq!(dep_chain.len(), 0);
        })
    }
    ...
    fn test_icebox_multiple_dependencies() {
        with_connection(|conn| {
            ...
            let mut got = conn.future_dependants(&[Cursor::new(10, 0u32)])?;
    ...
    fn test_icebox_chain() {
        with_connection(|conn| {
            ...
            let mut got = conn.future_dependants(&[Cursor::new(2, 0u32)])?;
