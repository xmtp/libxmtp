...
pub trait CheckPragmas<C: ConnectionExt> {
...
impl<C: ConnectionExt> CheckPragmas<C> for DbConnection<C> {
    fn busy_timeout(
        &self,
    ) -> Result<i32, crate::ConnectionError> {
        self.raw_query_read(|conn| {
            let BusyTimeout { timeout } = diesel::sql_query("PRAGMA busy_timeout")
