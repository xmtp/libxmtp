...
CREATE TABLE icebox_dependencies (
    ...
    PRIMARY KEY (envelope_sequence_id, envelope_originator_id, dependency_sequence_id, dependency_originator_id),
    -- when an envelope is deleted, also delete its dependency records
    FOREIGN KEY (envelope_sequence_id, envelope_originator_id) REFERENCES icebox(sequence_id, originator_id) ON DELETE CASCADE
);

CREATE INDEX idx_icebox_deps_lookup ON icebox_dependencies (dependency_sequence_id, dependency_originator_id);
