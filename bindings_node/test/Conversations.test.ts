...
describe('Conversations', () => {
  ...
  it('should find a message by ID', async () => {
    ...
    const messageId = await group.send(TextCodec.encode('gm!'), {
  ...
  it('should only stream group chat messages', async () => {
    ...
    await groupsList4[0].conversation.send(TextCodec.encode('gm3!'), {
      shouldPush: TextCodec.shouldPush(),
    })
    const message1 = await groupsList2[0].conversation.send(
      TextCodec.encode('gm!'),
  ...
  it('should only stream dm messages', async () => {
    ...
    await groupsList2[0].conversation.send(TextCodec.encode('gm!'), {
      shouldPush: TextCodec.shouldPush(),
    })
    await groupsList3[0].conversation.send(TextCodec.encode('gm2!'), {
      shouldPush: TextCodec.shouldPush(),
    })
    const message3 = await groupsList4[0].conversation.send(
      TextCodec.encode('gm3!'),
  ...
  it('stream should process dm messages from new installations without sync', async () => {
    ...
    await client_a_conversations[0].conversation.send(TextCodec.encode('gm!'), {
      shouldPush: TextCodec.shouldPush(),
    })
    ...
    await client_b_conversations[0].conversation.send(TextCodec.encode('b'), {
      shouldPush: TextCodec.shouldPush(),
    })
  ...
  it('should create initial group updated messages for added members', async () => {
    ...
    await group1.send(TextCodec.encode('gm1'), {
      shouldPush: TextCodec.shouldPush(),
    })
    await group1.removeMembersByInboxId([client2.inboxId()])
    await group1.send(TextCodec.encode('gm2'), {
      shouldPush: TextCodec.shouldPush(),
    })
    await group1.addMembersByInboxId([client2.inboxId()])
    await group1.send(TextCodec.encode('gm3'), {
      shouldPush: TextCodec.shouldPush(),
    })
    ...
    expect(messages2[0].content.type).toEqual(GroupUpdatedCodec.contentType())
    expect(messages2[1].content.type).toEqual(GroupUpdatedCodec.contentType())
    expect(messages2[2].content.type).toEqual(TextCodec.contentType())
    ...
    expect(messages3[0].content.type).toEqual(GroupUpdatedCodec.contentType())
    expect(messages3[1].content.type).toEqual(TextCodec.contentType())
    expect(messages3[2].content.type).toEqual(GroupUpdatedCodec.contentType())
    expect(messages3[3].content.type).toEqual(TextCodec.contentType())
    expect(messages3[4].content.type).toEqual(GroupUpdatedCodec.contentType())
    expect(messages3[5].content.type).toEqual(TextCodec.contentType())
    ...
    expect(messages4[0].content.type).toEqual(GroupUpdatedCodec.contentType())
    expect(messages4[1].content.type).toEqual(GroupUpdatedCodec.contentType())
    expect(messages4[2].content.type).toEqual(TextCodec.contentType())
  })
})
