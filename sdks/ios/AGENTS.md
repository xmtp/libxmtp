...
```bash
just ios build                          # xcframework + regenerate Sources/XMTPiOS/Libxmtp/xmtpv3.swift, via Nix
...
- `Sources/XMTPiOS/Libxmtp/xmtpv3.swift` is generated by uniffi and checked in. Never edit it by hand. `just ios build` regenerates it from the Rust source and copies it over; the release job does the same, so a hand edit is discarded. Change the doc comments in `bindings/mobile/src`, then run `just ios build` and commit the result.
