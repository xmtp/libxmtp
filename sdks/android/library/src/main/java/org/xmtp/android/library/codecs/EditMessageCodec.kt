...
data class EditMessageCodec(
    ...
    override fun encode(content: EditMessageRequest): EncodedContent {
        val ffiEditedContent = content.editedContent?.let { encoded ->
            uniffi.xmtpv3.FfiEncodedContent(
                typeId = encoded.type?.let { type ->
    ...
    override fun decode(content: EncodedContent): EditMessageRequest {
        ...
        val editedContent = decoded.editedContent?.let { ffiContent ->
            EncodedContent.newBuilder().apply {
                ...
                setContent(com.google.protobuf.ByteString.copyFrom(ffiContent.content))
            }.build()
        }
