...
import org.xmtp.kotlin.libxmtp.GroupPermissionPreconfiguration
    ...
    suspend fun findEnrichedMessage(messageId: String): DecodedMessageV2? =
        withContext(Dispatchers.IO) {
            try {
                ...
                XmtpLogging.logger.error("findEnrichedMessage failed", e.toString())
                null
            }
        }
            ...
            val conversationCallback =
                object : FfiConversationCallback {
                    ...
                    override fun onError(error: FfiException) {
                        XmtpLogging.logger.error("XMTP Conversation stream", error.toString())
                    }
        ...
        callbackFlow {
            val messageCallback =
                object : FfiMessageCallback {
                    ...
                    override fun onError(error: FfiException) {
                        XmtpLogging.logger.error("XMTP all message stream", error.toString())
                    }
