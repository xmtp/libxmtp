...
async function createProxy(backendUrl = process.env.XMTP_BACKEND_URL!) {
    ...
    async (mode, fault) => {
      const backend =
        fault === "drain" && process.env.XMTP_RECOVERY_BACKEND_BINARY
          ...
          : undefined;
      const proxy = await createProxy(backend?.url).catch(
        async (error: unknown) => {
          await backend?.close();
          throw error;
        },
      ...
      try {
        ...
        >["onValue"] = async (message) => {
          ...
          await group.sendText(`reply:${message.content}`);
        ...
        for (let cycle = 0; cycle < cycles; cycle += 1) {
          ...
          if (cycle === 0) {
            ...
            const installation = await createRegisteredClient(receiverSigner, {
      ...
      try {
        ...
        >["onValue"] = async (message) => {
          ...
          await conversation.sendText(
            message.content.replace("budget-request:", "budget-reply:"),
          );
        };
