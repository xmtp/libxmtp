...
describe("createStream terminal native recovery", () => {
...
describe("structured notification startup failure", () => {
  ...
  it.each(["none", "sync"] as const)(
    "ignores an old onValue storage rejection with %s mutation",
    async (mode) => {
      const h = makeHarness();
      ...
      old.onFail();
      await vi.advanceTimersByTimeAsync(10);
      pending.reject(new Error("[GroupError::Storage] old callback failed"));
      await vi.advanceTimersByTimeAsync(0);
      expect(onError).not.toHaveBeenCalled();
      expect(stream.isDone).toBe(false);
      expect(h.last().closer.end).not.toHaveBeenCalled();
    ...
    async (termination) => {
      ...
      if (termination === "error") {
        const cause = new Error(
          "[LocalDeliveryError::NetworkRecoveryExhausted] offline",
        );
