...
// Core reports exhausted network budgets and storage failures as terminal.
// Reopening here would bypass that boundary and hide the original failure.
const isTerminalNativeFailure = (error: unknown) => {
  if (
    error instanceof Error &&
    /^\[(?:LocalDeliveryError::(?:NetworkRecoveryExhausted|NetworkFailure)|(?:SubscribeError|GroupError|ClientError)::(?:Db|Storage)|GroupError::SqlKeyStore)\]/.test(
      error.message,
    )
  )
    return true;
...
export type StreamOptions<T = unknown, V = T> = {
   ...
   * Terminal native storage and network errors are not retried.
   * (default: 10)
   */
 ...
 *
 * Unexpected native closures can be retried. Terminal native storage and
 * network errors end the stream and reject iteration with the original cause.
 *
...
export const createStream = async <T = unknown, V = T>(
  ...
  // Core owns network recovery and its healthy reset. This separate finite
  // budget only restarts a native loop that closes without a terminal cause.
          ...
          if (isPromise(mutatedValue)) {
            void mutatedValue
              .then((mutatedValue) => {
                ...
                if (isCurrent() && mutatedValue !== undefined) {
                  asyncStream.push(mutatedValue);
                  return onValue?.(mutatedValue);
                }
              })
              .catch((error: unknown) => {
                if (isCurrent()) handleAsyncError(error);
              });
          } else {
