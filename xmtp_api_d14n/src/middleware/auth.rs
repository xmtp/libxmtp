...
use xmtp_common::{MaybeSend, MaybeSync, time::now_secs};
...
pub struct AuthMiddleware<C> {
    inner: C,
    handle: AuthHandle,
    callback: Option<Arc<dyn AuthCallback + Send + Sync + 'static>>,
}
...
impl<C> AuthMiddleware<C> {
    ...
    async fn get_credential(
        &self,
    ) -> Result<Option<&ArcSwap<Credential>>, Box<dyn std::error::Error + Send + Sync>> {
        let arc_swap = if let Some(callback) = &self.callback {
            let arc_swap = self
                ...
                .get_or_try_init(|| async {
                    ...
                    Ok::<_, Box<dyn std::error::Error + Send + Sync>>(arc_swap)
                })
                .await?;
            Some(arc_swap)
        } else {
