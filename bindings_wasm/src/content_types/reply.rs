...
pub fn encode_reply(reply: Reply) -> Result<Uint8Array, WasmError> {
  // Convert Reply to xmtp_content_types::reply::Reply
  let encoded =
    ReplyCodec::encode(reply.into()).map_err(|e| WasmError::from_error(ErrorCode::ContentType, e))?;
...
pub fn decode_reply(bytes: Uint8Array) -> Result<Reply, WasmError> {
  ...
  let reply =
    ReplyCodec::decode(encoded_content).map_err(|e| WasmError::from_error(ErrorCode::ContentType, e))?;
