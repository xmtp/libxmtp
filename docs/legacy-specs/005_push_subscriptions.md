...
**Provider facts that shaped the design.** APNs background pushes need `apns-push-type: background`, `apns-priority: 5`, and an `aps` dictionary holding only `content-available`. APNs returns `410 Unregistered` for dead tokens and `400 BadDeviceToken` for tokens that can belong to another environment. The provider JWT is ES256. Normal refresh is between 20 and 60 minutes; `403 ExpiredProviderToken` needs a new JWT and a resend. FCM HTTP v1 needs an OAuth2 token from a service account, rejects high-priority data-only messages to Apple devices, returns `UNREGISTERED` for dead tokens, and allows at most four collapse keys per device. FCM quota errors can have HTTP `429`, status `RESOURCE_EXHAUSTED`, and a typed `google.rpc.QuotaFailure` detail without an `FcmError` detail. FCM `500` and `503` responses can have no usable error body. FCM retries need at least 10 seconds, or 60 seconds for quota failures, and must honor longer `Retry-After` values within the local 300 second bound. Standard Webhooks defines `webhook-id`, `webhook-timestamp`, and `webhook-signature` over `id.timestamp.body`.
...
| --- | --- | --- | --- | --- | --- |
| APNs | HTTP/2 to the production or sandbox host from config, provider JWT (ES256, key id, team id) refreshed every 55 minutes, headers `apns-topic` = bundle id, `apns-push-type: background`, `apns-priority: 5`, `apns-collapse-id` = the base64 topic, body `{"aps":{"content-available":1},"topic":..,"sequence_id":..}` | `200` | `410 Unregistered`, `410 ExpiredToken` | mismatch: `400 BadDeviceToken`, `400 DeviceTokenNotForTopic`; rejected: other `400`, `403`, `404`, `405`, `413`, except the first `403 ExpiredProviderToken` triggers the credential refresh below | `429`, `500`, `503`, timeout, connection error |
| FCM | HTTP v1 `messages:send` with an OAuth2 service-account token, `data` = `{"topic": .., "sequence_id": ..}`, `android.priority: HIGH`, `apns.headers` `apns-priority: 5` and `apns-push-type: background`, `apns.payload.aps.content-available: 1`, no collapse key | `200` | typed `UNREGISTERED` | mismatch: typed `SENDER_ID_MISMATCH`; rejected: `INVALID_ARGUMENT`, `THIRD_PARTY_AUTH_ERROR`, other `4xx` except quota failures | HTTP `429`, typed `QuotaFailure` or `QUOTA_EXCEEDED`, HTTP `500` or `503` even without a usable body, typed `UNAVAILABLE` or `INTERNAL`, timeout, connection error |
...
On the HTTPS channel a `404` or `410` is retried like a transient failure; the recipient is terminal only when every attempt answered `404` or `410`. A mismatch outcome names a token that does not belong to this sender: it is one attempt, no delete, and its own counter, because an operator who changes `environment`, `bundle_id`, or the FCM project would otherwise delete every affected recipient in one sweep. Those recipients stop receiving pushes until the config is corrected, and expire on their own if it never is. FCM requires a typed `FcmError` detail for deletion or mismatch. HTTP `429` and typed `google.rpc.QuotaFailure` details are quota failures. Quota failures take precedence over other error details and need at least 60 seconds before retry. HTTP `500` and `503` are transient even without a usable body and take precedence over deletion or mismatch details. All other FCM transient failures need at least 10 seconds before retry. Longer `Retry-After` values are honored up to 300 seconds. A response with no recognized error or transient HTTP status is rejected. Android uses high priority because the app shows a notification on receipt; FCM downgrades senders that do not. Apple devices through FCM must use priority 5 or FCM rejects the message.

APNs `403 ExpiredProviderToken` invalidates only the JWT used by that request, not a newer cached JWT. The sender gets a fresh JWT and resends once immediately within the same dispatcher attempt and its original 10 second deadline. A repeated `403 ExpiredProviderToken` invalidates that JWT and is rejected without another resend or recipient deletion. Normal scheduled refresh remains every 55 minutes. Other `403` reasons remain rejected. This credential resend is the only exception to the APNs one second delay; it does not start a new dispatcher attempt or change `max_attempts`.
...
- **Rejecting all APNs `403` and FCM errors without typed `FcmError` details.** Revised with owner approval on 2026-09-15. APNs `ExpiredProviderToken` gets one immediate credential refresh and resend within the original deadline; a repeated expiry is rejected without deletion. FCM HTTP `429` and typed `QuotaFailure` use the 60 second quota minimum. HTTP `500` and `503` retry without a usable body. All other FCM transient failures use a 10 second minimum. Longer `Retry-After` values are honored up to 300 seconds. Deletion and mismatch still require a typed FCM detail.
...
- PUSH-060: WHEN sending through APNs THE SYSTEM SHALL use the configured host, a provider JWT for the configured key with normal refresh every 55 minutes and the credential-expiry exception in PUSH-061, `apns-topic` equal to `bundle_id`, `apns-push-type: background`, `apns-priority: 5`, `apns-collapse-id` equal to the base64 topic, and a body whose `aps` holds only `content-available: 1` beside `topic` and `sequence_id`.
- PUSH-061: WHEN APNs answers `200` THE SYSTEM SHALL count the delivery as delivered; WHEN it answers `410 Unregistered` or `410 ExpiredToken` THE SYSTEM SHALL treat the attempt as terminal; WHEN it answers `400 BadDeviceToken` or `400 DeviceTokenNotForTopic` THE SYSTEM SHALL treat it as mismatch; WHEN it answers `429`, `500`, `503`, a timeout, or a connection error THE SYSTEM SHALL treat it as transient. WHEN APNs answers `403 ExpiredProviderToken` THE SYSTEM SHALL invalidate only the cached JWT used by that request, obtain a fresh JWT, and resend once immediately within the same dispatcher attempt and its original 10 second deadline. A repeated `403 ExpiredProviderToken` SHALL invalidate that JWT and be rejected without another resend or recipient deletion. Every other status SHALL be rejected.
...
- PUSH-063: WHEN FCM answers `200` THE SYSTEM SHALL count the delivery as delivered. WHEN FCM answers HTTP `429`, a typed `google.rpc.QuotaFailure` detail, or typed `FcmError` code `QUOTA_EXCEEDED` THE SYSTEM SHALL treat it as a transient quota failure, including HTTP `429` with status `RESOURCE_EXHAUSTED` and no `FcmError` detail. WHEN FCM answers HTTP `500` or `503` THE SYSTEM SHALL treat it as transient even with an absent, malformed, or unreadable body. These transient HTTP statuses and quota failures SHALL take precedence over deletion or mismatch details. Otherwise, WHEN a typed `FcmError` detail gives `UNREGISTERED` THE SYSTEM SHALL treat it as terminal; WHEN it gives `SENDER_ID_MISMATCH` THE SYSTEM SHALL treat it as mismatch; WHEN it gives `UNAVAILABLE` or `INTERNAL`, or a timeout or connection error occurs, THE SYSTEM SHALL treat it as transient. Every other error SHALL be rejected. Only a typed `FcmError` detail SHALL authorize deletion or mismatch.
...
- PUSH-069: THE SYSTEM SHALL wait at least the channel's retry delay between dispatcher attempts on one delivery: 1 second on APNs and HTTPS; on FCM, 60 seconds after a quota failure defined in PUSH-063 and 10 seconds after any other transient failure. FCM SHALL honor a longer valid `Retry-After`, including on HTTP `503`, bounded to 300 seconds. Missing, malformed, or shorter values SHALL use the applicable minimum. The single immediate APNs credential resend in PUSH-061 SHALL stay within the original dispatcher attempt and deadline. A delivery whose next attempt is not yet due SHALL hold no in-flight permit. The configured `max_attempts` bound SHALL remain unchanged.
