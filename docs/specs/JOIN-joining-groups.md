...
JOIN-049 and JOIN-053 name the `GroupMembership` group context extension, which is what the client builds today. Section 8 describes the same statement as the `GROUP_MEMBERSHIP` dictionary component that GMOD-005 and META section 2 define. The two name the same membership statement in different models, and the rows are not amended here because the migration between them is not decided.
