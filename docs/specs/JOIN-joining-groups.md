...
Deleting a private key is the only thing that makes a Welcome already sent to it unreadable. Deleting early gives the shortest compromise window; deleting late keeps a Welcome that is still in flight openable. Publication of a replacement is confirmed when the backend's publish response carries a sequence id greater than 0 for the new package; `?API` owns the publish response and is expected to require that a stored envelope receives a sequence id greater than 0. From that confirmation, every superseded package is destroyed after a fixed delay, unless a Welcome the client has received is still waiting to be processed. A Welcome that is being held for a later attempt (JOIN-077, JOIN-057, JOIN-079) delays that destruction until it is completed or rejected.
...
The pointee is published as a `WelcomeMessage.V1` whose `installation_key` is the pointer's `destination`, so it is stored on the welcome topic for that identifier. `?TOPIC` owns the topic layout and is expected to require a 32-byte identifier for a welcome topic. The pointer is the only thing that carries the topic. That is what keeps the pointee unlinkable to the group and to its recipients, and it is why JOIN-029 forbids deriving `destination` from anything an observer already has.

The lowest sequence id at a topic is a meaningful rule only because the backend gives every topic a total order in which no later publication enters behind an earlier one. That ordering is owned by `?API`, which is expected to require that stored envelopes on one topic are totally ordered by sequence id and that the order is stable once assigned.
...
A Welcome hands over a group that already has history. Without a stated position in that history the joiner has two bad choices: read the whole topic and fail to decrypt everything before it was added, or read nothing and miss messages sent between the commit and its first poll. The sender therefore sets `message_cursor` to the sequence id, on the group's message topic, of the commit that added the recipient. MLS requires that commit to exist before the Welcome derived from it ([RFC 9420 §12.4.3](https://www.rfc-editor.org/rfc/rfc9420.html#section-12.4.3)), and the backend assigns sequence ids from one counter in publication order (`?API` is expected to require this), so the anchor is less than the Welcome's own sequence id or the Welcome is invalid. A client that installs a group reads its message topic from the anchor; reading earlier produces only messages it holds no secrets for.
...
The stored cursor, and the guarantee that a client advances it only over messages it has handled, are owned by `?PROC`, which is expected to require a durable per-group cursor that never moves backwards and that a client applies messages in sequence id order.
...
A Welcome tells the joiner who is in the group. That statement comes from the sender: the ratchet tree in the `GroupInfo`'s `ratchet_tree` extension ([RFC 9420 §12.4.3.3](https://www.rfc-editor.org/rfc/rfc9420.html#section-12.4.3.3)) lists every leaf node, and each leaf node's credential names an inbox. The joiner has no prior state to check that list against. What it has is the `GroupMembership` group context extension, whose `members` map names, for each inbox in the group, the identity sequence id at which that inbox's installations are to be checked, and whose `failed_installations` list names the installations a sender could not add. `?GMOD` owns that extension's identifier and the rule for writing it, and is expected to require that a commit adding an inbox records the sequence id it resolved and every installation it could not add. Identity state, its ordering, and what it means for an inbox to associate an installation at a sequence id are owned by `?IDENT`, which is expected to define the association log and the association state at a point in it.
...
The properties a two-party conversation must satisfy for JOIN-060 are owned by `?DMS`, which is expected to require that such a conversation names exactly the two inboxes it is between, that one of them is the joiner and the other is the adder under JOIN-025, and that its admin and super-admin lists are empty. The metadata field a group uses to state the minimum client version it requires is owned by `?META`. The user-visible state a conversation starts in is owned by `?CONS`, which is expected to require that a conversation a client did not ask to be added to starts in a state that does not present it as wanted, that the same state is reached on every installation of the inbox, and that this state gates whether the conversation is listed and streamed to the app. JOIN-060 requires the checks a conversation kind demands; which checks the kind used between an inbox's own installations demands is owned by `?SYNC`, which is expected to require that such a conversation is accepted only when the adder under JOIN-025 is the joiner's own inbox.
