...
| SPEC-093 | Default storage locations | The STORE spec MUST be exempt from SPEC-006 and SPEC-025 for the default database directories and the database file-name pattern it defines. | An app and its operators rely on `StorageLocation.Default` naming one location they can find, back up, and delete. |
