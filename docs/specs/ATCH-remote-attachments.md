...
| ATCH-079 | HTTP status is kept | When an upload or download fails with the cause `http_status` or `target_rejected` after the host answered, an SDK MUST report the final HTTP status code with the failure. | An app that cannot see the status cannot tell a 503 it can retry from a 403 it cannot. |
