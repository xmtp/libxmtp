...
| Pending attachment | The client's database record of one ciphertext that it created for upload. It holds the status (`waiting`, `uploading`, `complete`, or `failed`), the cause of the last failed upload, and the lease of a running upload. |
| Upload lease | The time until which a running upload holds a pending attachment. While the lease has not expired, no other upload of that pending attachment starts (ATCH-074). |
...
| ATCH-007 | The target enforces the signed request | An operator SHOULD configure only a storage target that rejects a PUT whose body's SHA-256 or length differs from the value its signature binds, and a PUT whose signature binds the create-only precondition when an object is at the key; that answers a PUT with a 2xx status only once it serves the object unchanged at the download URL of its content digest, and then keeps serving it there; that, where browser clients use the deployment, allows a cross-origin PUT with the signed headers and a cross-origin GET; and, for an S3 target, that accepts path-style requests, `{endpoint}/{bucket}/{object key}`, which is the form the backend signs. | ATCH-023 holds only on a target that enforces what the backend signs. On any other target, a member who knows a digest can store other bytes at it, and a target that answers before it serves the object lets the client delete its only staged copy under ATCH-037. |
| ATCH-073 | Signing credential sources | The backend MUST obtain the credentials it signs with from the source that `attachments.target.S3.credentials.kind` names: `static`, `default_chain`, `environment`, `profile`, `sso`, `process`, `web_identity`, `container`, `instance`, or `assume_role`, as the AWS SDK credential providers define them. When the kind is not one of these, or a field that kind requires is missing or empty, the backend MUST refuse to start and name the key under CONF-065. | An operator who runs the backend in a cloud account must not have to copy a long-lived secret into the configuration file. |
| ATCH-075 | Key prefix form | When the configured key prefix is not empty and contains a character other than an ASCII letter, digit, `_`, `-`, `.`, or `/`, begins with `/`, contains `//`, or has a `.` or `..` segment, the backend MUST refuse to start and name the key under CONF-065. | A prefix with other characters, or with empty or dot segments, signs an object key that differs from the one the operator maps the base URL to. |
| ATCH-081 | S3 target names | When the configured S3 region is empty or contains a whitespace character, or the configured bucket is empty, contains `/`, or is `.` or `..`, the backend MUST refuse to start and name the key under CONF-065. | A request signed for an empty region, or for a bucket that is not one path segment, fails at every target, so the backend would start but every upload would fail. |
...
| ATCH-024 | The client sends the request as signed | When the client uploads, it MUST send a PUT request to the `url` of the `CreateUploadResponse`, with every header of `headers` and its value, and the staged ciphertext as the body. A header that the client's HTTP transport sets from the body, such as `content-length`, MUST have the value `headers` gives. When `method` is not `PUT`, the client MUST end the upload as `failed` with the cause `target_rejected` without sending a request to the target. | The target refuses a request that differs from its signature. |
| ATCH-025 | Upload outcome | The client MUST end an upload as `complete` when the target answers the signed request with a 2xx status or 412, and as `failed` with the cause ATCH-060 names on any other answer or transport failure, including a timeout under ATCH-070, and when `CreateUpload` fails or the client cannot read or write the staged ciphertext or the record. | A 412 is a stored object under ATCH-023, so reporting it as a failure makes an app retry for ever. |
...
| ATCH-029 | Permanent rejections are not resent | When an upload has ended as `failed` because `CreateUpload` answered `INVALID_ARGUMENT`, `OUT_OF_RANGE`, or `UNIMPLEMENTED`, the client MUST record the cause `backend_rejected` in the pending attachment's record. While the record holds that cause, every client on the same database MUST end each request to upload that pending attachment as `failed` with that cause, without changing the record, emitting an event, or sending a request. | API-284: the same request fails the same way, and a retry loop, or a restart, loads the deployment for nothing. |
...
A client holds one pending attachment for each content digest, as a record in its database. The record carries the status, the cause of the last failed upload, and the upload lease, so every client on the same database reports the same status, also after the process ends and a client is created again. An app can list the pending attachments, or ask for one by its remote attachment, and can upload it many times: after a failure, from two places or two clients at once, or after completion. A pending attachment is dropped with its record and any staged ciphertext when it reaches a maximum age, one day unless the app sets another, unless an upload of it is running. The upload table says what each request does. The `attachment.upload_*` events report each change of status that the client makes (EVENT section 1).
...
The upload table: what the client does when an app asks to upload a pending attachment, by its status at that moment, read under ATCH-074.

| Status | What the client does |
| --- | --- |
| `waiting`, or `failed` with a cause other than `backend_rejected` | Takes the upload lease and sets the status to `uploading` in one change of the record, then starts an upload, which begins with `CreateUpload` unless ATCH-026 or ATCH-036 ends it first |
| `failed` with the cause `backend_rejected` | Sends no request and does not change the record; the request ends as `failed` with the cause `backend_rejected` (ATCH-029) |
| `uploading` | Sends no request; the request ends with the running upload's outcome, whichever client on the database runs it |
| `complete` | Sends no request; the request ends successfully |

| ID | Title | Requirement | Why |
| --- | --- | --- | --- |
| ATCH-030 | Creation preconditions | If the snapshot does not offer attachments, or the ciphertext of the source would be longer than the snapshot's `max_upload_bytes`, or the source's `filename` and `mime_type` would make the encoded content's retained fields exceed the limit of ATCH-051, then the client MUST fail creation with the cause ATCH-060 names, before it writes any file. | An app that sends the remote attachment of an object that can never be stored sends a message no recipient can open. |
...
| ATCH-033 | Failed creation leaves nothing | If creation fails, then the client MUST leave no plaintext file, no staged ciphertext, no pending attachment record, and no local attachment record for it. The client MUST write the plaintext file and the staged ciphertext outside their final paths and put each in place only by an atomic move, so creation that the end of the process interrupts leaves no partial file at either path. | An app retries creation; a leftover partial file is a file ATCH-052 would return as complete. |
...
| ATCH-034 | Status transitions | The client MUST keep a pending attachment's status in its record: `waiting` at creation, then only the changes the upload table states, and the outcome of an upload under ATCH-025, ATCH-026, ATCH-029, ATCH-036, ATCH-047, or ATCH-071 when the upload ends. After `complete`, the status MUST NOT change. | Two concurrent PUTs of one object race the create-only precondition for no gain; a completed upload repeated by a retry loop must not fail. |
| ATCH-074 | Upload lease | The client MUST set `uploading` only in the same atomic change of the record that takes the upload lease, and only when no other upload holds an unexpired lease; while the upload runs, it MUST extend the lease to 120 seconds after the current time at least every 30 seconds. A record whose status is `uploading` and whose lease has expired holds an interrupted upload, which has no outcome: every client MUST read its status as `waiting` and MUST NOT emit an event for it. An upload whose lease has expired or was taken by another upload MUST NOT change the record or delete the staged ciphertext, and the request it serves MUST end as the upload table states for `uploading`. | Two clients on one database would otherwise run two uploads of one object, a lease left by a process that ended would block the upload for ever, and a process that resumes after suspension would overwrite the state of the upload that replaced it. |
| ATCH-035 | One pending attachment per digest | The client MUST hold at most one pending attachment record for each content digest, and every handle an SDK returns for that digest, from any client on the same database, MUST report the status of that record and share its uploads. | Two handles or two clients would run two uploads of one staged ciphertext, and the one that completes first deletes the file under the other (ATCH-037). |
...
| ATCH-037 | Staged ciphertext is released | When a pending attachment becomes `complete`, the client MUST record `complete` and then delete its staged ciphertext. | Every sent attachment would otherwise occupy its size twice on the device. |
...
| ATCH-066 | Pending attachments persist | The client MUST keep each pending attachment record, with its status and the cause of its last failed upload, across the end of the process until ATCH-047 or ATCH-068 removes it. A client created later on the same database, and any other client on it at the same time, MUST hold the pending attachment with that status, read under ATCH-074. | An app restarted after an optimistic send would hold a message whose object nothing uploads, would resend an upload the backend refused (ATCH-029), and could not tell a completed upload from one that never ran. |
...
| ATCH-068 | Pending attachments expire | When a pending attachment was created more than the app's `max_pending_age_seconds` ago, or 86400 seconds when the app sets none, and no upload of it holds an unexpired lease (ATCH-074), the client MUST NOT list or return it, and MUST delete its staged ciphertext, if any, and its record at the next client creation. While the client runs, unless the app disabled its background workers or its attachment cleanup worker, the client MUST also delete them within 3600 seconds. The client MUST NOT delete a pending attachment's record or staged ciphertext while an upload of it holds an unexpired lease. | Staged ciphertext that nobody uploads occupies its size on the device for ever. |
...
| ATCH-080 | Explicit layout | When a client is created with an `explicit` `StorageLocation`, it MUST open the database at `db_path` and use `attachments_dir` as the attachments directory, without sending a request and without requiring the inbox id; ATCH-040 and ATCH-069 do not apply to it. | An app that manages its own paths, or restores a cached identity, must not depend on the network or on a known inbox id to open its database. |
...
| ATCH-077 | Private local files | Where the platform and the file system support POSIX file permissions, the client MUST create every directory and file it writes under the attachments directory, including staged ciphertext and partial files, and every directory it creates for a `data_dir` location (ATCH-040), including the `data_dir` directory itself when it is missing, and the record of deployment identifiers, with access for the owning user only: mode `0700` for a directory and `0600` for a file. | On a shared host, another local user would read decrypted attachments that the encrypted database does not expose. |
| ATCH-041 | Attachment key | The client MUST set the attachment key of a remote attachment to the lowercase hexadecimal SHA-256 of the 108-byte string made of the raw bytes, not their text encodings, of its 32-byte content digest, its 32-byte `secret`, its 32-byte `salt`, and its 12-byte `nonce`, in that order. | A key from the URL alone lets a sender who reuses another message's URL point the recipient at that message's file. |
...
| ATCH-047 | Local deletion | When an app asks the client to delete the local files of a remote attachment, the client MUST end each running upload of its content digest and each download that this client runs to a plaintext path under its attachment key directory as `failed` with the cause `deleted`, and then delete its attachment key directory, its staged ciphertext, its pending attachment record, and every local attachment record whose path is under that attachment key directory. The deletion MUST also end a download to a plaintext path under that directory that starts in this client while the deletion runs. An upload of that content digest that another client on the same database runs MUST end as `failed` with the cause `deleted` no later than its next lease extension or its outcome, when it finds the record gone. | A running download that completes after the deletion puts the file back; a record left for a file the deletion removed lists an attachment the app cannot open. |
...
| ATCH-063 | Records follow the files | The client MUST add the local attachment record right after it makes a plaintext file readable under ATCH-032 or ATCH-051, and MUST remove the record right after it deletes the file under ATCH-047. | A record without a file lists an attachment the app cannot open; a file without a record is never cleaned up. |
| ATCH-076 | Crash recovery of local files | At each client creation, before it lists local or pending attachments or answers a download, the client MUST add a local attachment record, with `created_at_ns` from the file's modification time, for each plaintext file under an attachment key directory that has none; MUST remove each local attachment record whose file is missing; and MUST delete each staged ciphertext that has no pending attachment record and was last modified more than 3600 seconds ago, or more than the pending attachment age of ATCH-068 when that is shorter, and each staged ciphertext whose record is `complete`. It MUST NOT delete a plaintext file (ATCH-046). | A process that ends between a file operation and its record leaves a file that no listing shows, or a record for a file the app cannot open; a staged file with no record is never uploaded or released. |
...
| ATCH-051 | Verified before readable | The client MUST write a download's plaintext outside the plaintext path and MUST put a file at the path only by an atomic move, after the ciphertext passes the digest check and the decryption of CTYPE-015 and decodes as `xmtp.org/attachment:1.0` encoded content. A download that fails, or that the end of the process interrupts, MUST leave no file at the path. The client MUST NOT treat as `xmtp.org/attachment:1.0` encoded content an envelope whose `type` field, `mimeType` and `filename` parameter entries, and `compression` field together take more than 65536 serialized bytes. | A partial or forged file at the path is shown by every later reader as the attachment. The client holds these fields in memory while content streams to disk; no file name or MIME type approaches this size, and an unbounded one is a memory attack. |
...
enum AttachmentFailureCause {
  ...
  "credential",           // CreateUpload failed with a client credential failure of AUTH section 6 (ATCH-061), or with PERMISSION_DENIED (ATCH-078)
  ...
  "target_rejected",      // the CreateUpload response names a method other than PUT, the upload PUT received a redirect, or the storage target answered a status other than 2xx or 412
...
| `insecure_url` for a download | Only when the URL is `http` to `localhost` or a loopback address and the app sets `allow_private_network`; otherwise no |
| `insecure_url` for an upload | No. The backend signed a URL that ATCH-071 forbids; it fails again until the operator fixes the storage endpoint |
...
| ATCH-078 | Missing scope | When `CreateUpload` answers `PERMISSION_DENIED`, the client MUST end the upload as `failed` with the cause `credential`, marked as a missing scope (AUTH-013) that retrying with the same credential cannot cure, and MUST NOT record it under ATCH-029. | An app that reads a scope rejection as a transient outage retries for ever; one that records it as a permanent rejection cannot upload after it obtains a credential with the scope. |
| ATCH-079 | HTTP status is kept | When an upload or download fails with the cause `http_status` or `target_rejected` after the host answered, an SDK MUST report the final HTTP status code with the failure. Where the client does not follow redirects itself and the platform reports a redirect answer without its status, the SDK MUST report that failure without a status. | An app that cannot see the status cannot tell a 503 it can retry from a 403 it cannot. |
...
A remote attachment forwarded unchanged in another message shares its plaintext path, and one that carries the same key material with another `filename` shares its attachment key directory, so deleting the local files for one message deletes them for both, and the next download fetches them again.
...
In a browser, the SQLite store keeps every database in one origin-private pool, so the client names the inbox's database by its `{data_dir}/{deployment}/{inbox_id}/` path but cannot place it in that directory. An app that removes the directory also removes the attachments but not the database. In a browser, ATCH-054 rejects `localhost` names and private IP literals in a download URL while `allow_private_network` is false. The client cannot check the address to which another hostname resolves before the browser connects; the browser's private-network rules apply to those hostnames. ATCH-055 ends the download on every redirect, including an opaque redirect. A browser client cannot set a connect timeout or observe upload progress of a fetch PUT, so its PUT has no deadline beyond the browser's own. The client can observe response reads of a download GET and applies ATCH-070 to them. A download host that sends no CORS headers cannot serve a browser client.

Each client emits the `attachment.*` events only for the changes it makes. A second client on the same database, such as an app and its notification extension, reads the recorded status of a pending attachment but receives no event for an upload that the other client runs. The upload lease depends on the device clock and on a running process: a clock that jumps forward, or a platform that suspends the process for more than 120 seconds, expires a running upload's lease, so a second upload of the same object can start. The first upload then leaves the record to the second (ATCH-074), and when both PUTs reach the target, the create-only precondition (ATCH-023) ends one of them with 412. A deletion ends only the downloads that the deleting client runs. A download of the same remote attachment that another client on the same database runs can complete after the deletion and put the plaintext file and its local attachment record back (ATCH-051, ATCH-063); the app can delete the local files again after that download ends.

The client does not change the permissions of an attachments directory the app supplies; an app that supplies one that other local users can write gives them control of the files below it. A managed directory that already exists and that another user owns fails with `local_storage` (ATCH-077), so a file system that reports a fixed owner for every file, such as Android shared storage or some network mounts, cannot hold a client-managed attachments directory; app-private storage is not affected.

A browser reports a redirect answer to a fetch with manual redirect handling as an opaque redirect without its status, so a browser upload that ends on a redirect fails with `target_rejected` and no status.

On a file system without POSIX permissions, such as NTFS on Windows, ATCH-077 does not apply: the files the client writes inherit the access control list of the directory the app supplies. An app keeps attachments private to its user by placing the data directory or attachments directory under that user's profile, such as `%LOCALAPPDATA%`, whose inherited access list grants only that user, administrators, and the system.
