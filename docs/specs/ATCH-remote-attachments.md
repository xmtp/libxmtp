...
| ATCH-047 | Local deletion | When an app asks the client to delete the local files of a remote attachment, the client MUST end each running upload of its content digest and each running download to a plaintext path under its attachment key directory as `failed` with the cause `deleted`, and then delete its attachment key directory, its staged ciphertext, its pending attachment record, and every local attachment record whose path is under that attachment key directory. The deletion MUST also end a download to a plaintext path under that directory that starts while the deletion runs. An upload of that content digest that another client on the same database runs MUST end as `failed` with the cause `deleted` no later than its next lease extension or its outcome, when it finds the record gone. | A running download that completes after the deletion puts the file back; a record left for a file the deletion removed lists an attachment the app cannot open. |
