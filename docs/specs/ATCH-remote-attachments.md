...
| ATCH-077 | Private local files | Where the platform and the file system support POSIX file permissions, the client MUST create every directory and file it writes under the attachments directory, including staged ciphertext and partial files, with access for the owning user only: mode `0700` for a directory and `0600` for a file. | On a shared host, another local user would read decrypted attachments that the encrypted database does not expose. |
