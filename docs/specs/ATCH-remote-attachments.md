...
| ATCH-029 | Permanent rejections are not resent | When an upload has ended as `failed` because `CreateUpload` answered `INVALID_ARGUMENT`, `OUT_OF_RANGE`, or `UNIMPLEMENTED`, the client MUST record the cause `backend_rejected` in the pending attachment's record, and every client on the same database MUST end each later upload of that pending attachment as `failed` with that cause without sending a request. | API-284: the same request fails the same way, and a retry loop, or a restart, loads the deployment for nothing. |
...
| --- | --- |
| `waiting` or `failed` | Takes the upload lease and sets the status to `uploading` in one change of the record, then starts an upload, which begins with `CreateUpload` unless ATCH-026, ATCH-029, or ATCH-036 ends it first |
...
| ATCH-037 | Staged ciphertext is released | When a pending attachment becomes `complete`, the client MUST record `complete` and then delete its staged ciphertext. The record stays until ATCH-047 or ATCH-068 removes it. | Every sent attachment would otherwise occupy its size twice on the device. |
...
| ATCH-068 | Pending attachments expire | When a pending attachment was created more than the app's `max_pending_age_seconds` ago, or 86400 seconds when the app sets none, and no upload of it holds an unexpired lease (ATCH-074), the client MUST NOT list or return it, and MUST delete its staged ciphertext, if any, and its record at the next client creation, or within 3600 seconds while the client runs. The client MUST check the lease and delete the record in one atomic change of the record. | Staged ciphertext that nobody uploads occupies its size on the device for ever. |
...
Each client emits the `attachment.*` events only for the changes it makes. A second client on the same database, such as an app and its notification extension, reads the recorded status of a pending attachment but receives no event for an upload that the other client runs. The upload lease depends on the device clock: a clock that jumps forward can expire a running upload's lease, so a second upload of the same object can start, and the target's create-only precondition (ATCH-023) ends one of them with 412, which is `complete`.
