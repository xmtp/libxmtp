...
Out of scope: credentials and which requests need one (`?AUTH`); the values of the published limits and how a client sizes its requests to them (`CONF`); the topic layout (`TOPIC`); push registration and delivery (`?PUSH`); retention enforcement, readiness, and health (`?OPS`); the association log an identity update is checked against (`?IDENT`); the MLS objects a key package and a Welcome carry (`JOIN`); the commit-log entry (`?FORK`); and what a client does with a frame once received, including its positions on a topic (`?PROC`).
...
| `?PROC` | Expected to own how a client advances a cursor from the envelopes it receives, and how it recovers from a closed stream. |
| `?SEND` | Expected to own the client's publish retry: an unanswered publish is retried with the same bytes. |
...
The backend assigns every stored envelope a sequence id from one counter shared by every topic. The counter only increases, so a sequence id orders envelopes on one topic and also says which of two envelopes on different topics was allocated first. Sequence ids on one topic are not contiguous: a rejected or abandoned publish consumes ids that never appear, and other topics take the ids in between.
...
A deployment may answer some reads from a lagging copy of its store. `QueryNewest`, a stream's catch-up target, and `GetInboxIds` can therefore answer from an earlier point in time than a publish response the client already holds; each still returns a prefix of every topic. Only `Query` is promised to read the client's own writes (API-202). `?PROC` is expected to require that a client which needs its own publish visible fetches it with `Query` rather than waiting on a stream.

A sequence id can be allocated to a publish that then fails, so at any moment some ids below the newest are neither visible nor ever will be, and others belong to publishes still in progress. The closed allocation boundary separates the two: at or below it, every publish has finished. A reader that must never skip an envelope, such as the push dispatcher (`?PUSH`), reads only up to the boundary; a stream holds a gap open until the boundary passes it.

| ID | Title | Requirement | Why |
| --- | --- | --- | --- |
| API-200 | One counter for every topic | The backend MUST assign every stored envelope a sequence id greater than 0 and not greater than 9223372036854775807, unique across all topics, from one counter that never decreases, so that an envelope published after another envelope became visible carries a greater sequence id than that envelope, whatever their topics. A sequence id MUST NOT change once assigned and MUST NOT be assigned to a second envelope. | A join anchor and an identity reference compare sequence ids across topics (JOIN-037, JOIN-050); with more than one counter, or an id that moves, those comparisons mean nothing. |
...
A client publishes a `ClientEnvelope`. The backend decodes it, encodes the decoded message again with its own encoder, stores those bytes, and returns them inside a `ServerEnvelope` with the metadata it assigned. The payload byte fields, which carry MLS objects and signed structures, are returned byte for byte; only the protobuf framing around them is re-encoded, so the stored length can differ from the wire length. The message hash is computed over the stored bytes, so it is a stable identity for the same input and an opaque value to the client: a hash a client computes over its own encoding need not match, and `?PROC` is expected to require that a client never rejects a received envelope because a hash it computed differs from `message_hash`.
...
| Field | Value |
| --- | --- |
| `cursor.sequence_id` | The envelope's sequence id under API-200. |
...
| Identity update | `IdentityUpdate` with `inbox_id` accepted by TOPIC-002 | That every signature verifies, and that the update applies to the association state built from every envelope on the inbox's identity topic, under the rules `?IDENT` states | `REASON_MALFORMED_PAYLOAD` when TOPIC-002 rejects it, `REASON_INVALID_SIGNATURE` when a signature does not verify, `REASON_INVALID_IDENTITY_UPDATE` otherwise | That the caller is authorized to act for the inbox |
...
`Query` reads envelopes above a cursor on each of many topics, in one page bounded by a total limit. The page is cut by the limit and nothing else: for every topic it includes, it holds that topic's eligible envelopes from the lowest up, with none left out in between, so that a client which moves a topic's cursor to the highest sequence id it received has skipped nothing. `QueryNewest` reads the one newest envelope, or only its metadata, on each of many topics; a topic with nothing on it is left out rather than answered with an empty result, so a client matches results to topics by the topic each result carries. A key package topic is read only through `QueryNewest`, because a client wants the current package and never the history. A client pages by moving each topic's cursor to the highest sequence id it received on that topic and asking again while `has_more` is true; `?PROC` is expected to require that a client advances a topic's cursor only to a sequence id it received on that topic (PROC-003 states the sources it must not use).
...
Publish admission establishes only what the admission table says. A stored group message is not evidence that its sender is a member; a stored Welcome is not evidence that its destination exists; a stored commit-log entry is not evidence that its signature verifies. Storage and the recipient's processing capacity can be consumed by anyone who can reach the backend, and only a credential (`?AUTH`) limits who that is.
