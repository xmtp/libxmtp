...
| AUTH-035 | A shared handle shares its lockout | When client instances share one credential handle, the failure count and the lockout of AUTH-023 MUST belong to the handle, so a failure from any sharing client counts toward one lockout and the lockout applies to every sharing client. | Per-client counts on one credential would let the clients together send many times the limit before any lockout. |
...
| AUTH-025 | Streams wait out a lockout | While the client is in lockout, a subscription stream that needs to reconnect MUST wait for the cool-down and then reconnect within the recovery episode limits in PROC-038, and MUST NOT end solely because the client entered lockout. When a reconnect fails because a credential is missing or rejected without a refresh callback, the affected stream MUST end with that failure. Other response failures follow the bounded recovery rule in PROC-038. | A temporary credential failure must permit recovery on the same stream. |
...
The lockout is per credential handle, not per deployment (AUTH-035). A client that is recreated with a new handle starts its count again.
