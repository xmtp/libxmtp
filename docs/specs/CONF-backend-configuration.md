...
| CONF-076 | Offline creation ignores a moved URL | Where an app creates a client that may not reach the backend, the client MUST NOT fail creation because the stored copy's backend URL differs from the client's backend URL. | An app that moved its backend URL must still open offline; CONF-077 re-checks before the first request. |
| CONF-077 | Offline URL re-check before first request | When a client created under CONF-034 on a database whose stored copy carries a backend URL that differs from the client's backend URL is about to send its first request after creation, it MUST first send `GetConfiguration`, apply CONF-071 and CONF-030, and store the answer with the new URL. | Otherwise a database for one deployment sends its identity and group traffic to another deployment before any identifier check. |
