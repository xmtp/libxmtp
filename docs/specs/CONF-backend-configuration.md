...
| CONF-043 | Advisory group size | When the number of distinct inboxes in a group's `GroupMembership` extension plus the inboxes a change would add is greater than `max_group_members`, the client MUST reject the change before it builds a commit. | |
