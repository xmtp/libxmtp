...
Out of scope: publish atomicity and duplicate detection on the backend (API); receipt and ordered processing of the echo (PROC); what a commit contains and how it is validated (GMOD); building and wrapping Welcomes (JOIN); the guard comparison of an app-data write (META-065); the content encoding inside a message (CTYPE); consent recorded on send (CONS); and push flags (PUSH).
...
Two defaults sit at the SDK boundary. A string handed to a send with no content type is text, so every SDK encodes it under the catalogue text type (CTYPE-014); the default applies to a send request, not to received content with a missing or unknown identifier (CTYPE-008). The push flag on the envelope (API-210) defaults to the catalogue value of the content type under CTYPE-010; a value the app supplies, including `false`, replaces that default, and an omitted options object or an options object with no push field leaves it in effect.
...
| SEND-020 | A string sends as text | When an app sends a string with no content type, the SDK MUST encode it as `xmtp.org/text:1.0` under CTYPE-014 before it queues the message. | Two SDKs that default differently deliver the same message under different identifiers, and a recipient decodes one and not the other. |
| SEND-021 | A supplied push value wins | When an app supplies a push value with a send, the client and SDK MUST set the envelope's `should_push` to that value, including `false`, in place of the content type's catalogue value. | An app that suppresses a notification and is overridden wakes recipients for a message it chose to send silently. |
...
API-284 prohibits an unchanged request after `OUT_OF_RANGE`, while API section 7 permits that status after a publish committed. SEND-007 preserves the attempt without requiring a prohibited resend. API-290 is the recovery contract: a `Query` of the topic after that status returns each committed envelope with its original receipt, and an envelope it does not return was not stored. The client retains the exact envelope bytes, including required follow-up Welcome batches, while any outcome is unknown, and neither replaces, re-encrypts, nor terminally fails that work until the recovery read establishes its outcome.
...
| SEND-007 | Preserve bytes while outcome is unknown | While a group attempt or required follow-up Welcome batch has an unknown publish outcome, the client MUST retain its exact prepared bytes and MUST NOT replace or re-encrypt them. Any retry permitted by API-284 MUST reuse those bytes; an `OUT_OF_RANGE` result MUST use the recovery read of API-290 instead of an unchanged request. | An ambiguous response can follow a committed publish. |
