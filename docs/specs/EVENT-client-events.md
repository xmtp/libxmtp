...
| EVENT-057 | Ending stops callback starts | After an app ends a subscription, an SDK MUST NOT admit another callback for it past the subscription's start check, and MUST complete a pending iterator read of it as ended. | A callback admitted after end can act on state the app has released. |
