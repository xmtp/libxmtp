...
| PROC-043 | Retryable error close | When a message or conversation stream ends with an error, the SDK MUST mark it retryable for PROC-038 and PROC-040, and not retryable for AUTH-025, CONF-075, PROC-031, and PROC-033. | An app needs to decide whether to reopen without retrying a terminal error. |
