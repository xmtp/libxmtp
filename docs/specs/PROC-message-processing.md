...
| PROC-044 | Ordered connection states | When an app subscribes to a message or conversation stream's connection-state changes, the SDK MUST deliver the state at subscription and then each change in order, using the connection state exposed under PROC-023. | A missing or reordered state can leave the app showing the wrong connection status. |
