...
| STORE-002 | Default directory on Apple platforms | Where an SDK runs on iOS or macOS and the app asks for the default location, the SDK MUST place the file in the `xmtp` directory inside a folder named by the app name the app supplies when it creates the client, or by the app's bundle identifier when it supplies none, in the app's Application Support directory. | Documents is visible to the user and can be moved or deleted from the Files app; Application Support holds app data the user does not manage. |
...
| STORE-006 | No default elsewhere | Where an SDK runs on a platform that STORE-002 to STORE-005 do not name, or on iOS or macOS in a process with neither a supplied app name nor a bundle identifier, a request for the default location MUST fail with a typed error before the SDK opens a file. | A guessed directory puts databases where the app cannot find or back them up. |
...
| STORE-008 | Safe path components | If a storage label contains a path separator (`/` or `\`) or a NUL character, or an app name supplied for STORE-002 is empty, is `.` or `..`, or contains a path separator or a NUL character, then the SDK MUST fail with a typed error before it opens a file. | A label or app name taken from outside the app could otherwise place the database outside the intended directory, or open another database. |

The SDK uses the storage label and the app name unchanged. On a file system that ignores letter case, values that differ only in case name the same file or directory.
