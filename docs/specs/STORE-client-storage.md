...
| STORE-008 | Safe storage label | If a storage label contains a path separator (`/` or `\`) or a NUL character, then the SDK MUST fail with a typed error before it opens a file. | A label taken from outside the app could otherwise place the database outside the intended directory, or open another database. |
