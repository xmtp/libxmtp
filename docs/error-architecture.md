...
### `#[error_code(internal)]` -- internal types

Marks an error type as internal, meaning it is not reachable from the SDK binding layer. Internal types are filtered out of the auto-generated error glossary:
...
### 5. Mark internal types

If the error type is purely internal (not reachable from SDK bindings), mark it with `#[error_code(internal)]`.

### 6. Regenerate the glossary

Run `dev/gen-error-glossary` to regenerate `docs/error_glossary.md`.
...
6. **Doc comments are required.** Every variant needs a `///` doc comment describing the error. The derive macro enforces this at compile time. These comments are used to auto-generate the error glossary.
