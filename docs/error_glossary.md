...
**31 error types** across **10 crates** with **355 total error codes**.
...
| Error Code | Description |
|:-----------|:------------|
| `CatchUpError::Unsupported` | Bidi unsupported. The backend refuses the bidi surface — the latch-worthy verdict ([`open_is_backend_refusal`]). The dispatch layer falls back to the legacy sync; this never escapes [`Client::catch_up_to_live`]. Not retryable. |
