...
| `crates/xmtp_mls/src/groups/mls_sync/publish/tests.rs` | `groups::mls_sync::publish::tests::send_completes_after_queued_state_changes` | XMTP async; more queued metadata commits than the retry limit; final metadata, authenticator, and exactly-once message assertions | `GINLINE-REQ-099` |
