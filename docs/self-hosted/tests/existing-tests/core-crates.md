...
| crates/xmtp_db/src/encrypted_store/group_message/tests.rs | `encrypted_store::group_message::tests::it_orders_messages_by_sent` | `xmtp_common::test`; ascending, descending, canonical group activity under clock skew, publication rollback, and idempotent delivery cursor. | `CORE-REQ-097` |
