...
This index contains 220 source-defined test declarations. All indexed source files are connected to their crate module trees. Listed target and feature gates still apply. A parameterized declaration is one row.
...
| `crates/xmtp_mls/src/subscriptions/incoming/controller/tests.rs` | `subscriptions::incoming::controller::tests::an_invalid_supported_head_does_not_hold_a_later_valid_message` | XMTP async; backend; malformed and reused-generation heads followed by valid commit and message | `MLS-REQ-120` |
...
| `crates/xmtp_mls/src/subscriptions/incoming/controller/tests.rs` | `subscriptions::incoming::controller::tests::concurrent_stream_setup_keeps_bidi_as_the_last_factory` | Native XMTP async; thread barrier; concurrent generic and bidi setup; no later downgrade | `MLS-REQ-147` |
...
| `crates/xmtp_mls/src/subscriptions/incoming/controller/tests.rs` | `subscriptions::incoming::controller::tests::a_selected_transport_handle_keeps_its_factory_until_the_first_lease` | XMTP sync; selected handle held across factory command and first lease; controlled idle check and final release | `MLS-REQ-164` |
