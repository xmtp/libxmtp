...
pub async fn create_tls_channel(address: String, limit: u64) -> Result<Channel, GrpcBuilderError> {
    ...
    let endpoint = Channel::from_shared(address.clone()).map_err(GrpcBuilderError::from)?; // Ensure consistent error types

    let retry_strategy = ExponentialBackoff::from_millis(500)
        .max_delay(Duration::from_secs(10))
        .take(5)
        .map(jitter); // Adds randomized delay to reduce thundering herd effects
...
pub mod tests {
    ...
    #[tokio::test]
    async fn tls_channel_connects_to_dev() {
        let result = create_tls_channel(DEV_ADDRESS.to_string(), 1000).await;
        assert!(
            result.is_ok(),
            "Expected successful TLS connection, got error: {:?}",
            result.err()
        );
    }

    #[tokio::test]
    async fn tls_channel_retries_then_errors() {
        // Port unlikely to be open to trigger retries
        let unreachable = "https://127.0.0.1:59999";

        let start = std::time::Instant::now();
        let result = create_tls_channel(unreachable.to_string(), 1000).await;
        let elapsed = start.elapsed();

        assert!(result.is_err(), "Expected connection failure");

        // Should reflect backoff behavior roughly
        assert!(
            elapsed >= std::time::Duration::from_millis(500) * 5,
            "Retry delays should accumulate"
        );
    }

    #[tokio::test]
    async fn non_tls_channel_retries_then_errors() {
        let mut builder = Client::builder();
        builder.set_host("http://127.0.0.1:59999".into());
        builder.set_tls(false);

        let result = builder.build().await;
        assert!(
            result.is_err(),
            "Expected non-TLS connection failure after retries"
        );
    }
}
