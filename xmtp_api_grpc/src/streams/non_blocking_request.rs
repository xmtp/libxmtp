...
pub async fn send<F>(this: NonBlockingStreamRequest<F>) -> Result<Response<NonBlockingWebStream<NonBlockingStreamRequest<F>, tonic::Streaming<Bytes>>>, Status>
where
    F: Future<Output = Result<Response<tonic::Streaming<Bytes>>, Status>> + Send,
{
    let s = this.await?;
    let (meta, body, ext) = s.into_parts();
    let body = NonBlockingWebStream::started(body);
    Ok(Response::from_parts(meta, body, ext))
}


#[cfg(target_arch = "wasm32")]
pub async fn send<F>(mut this: NonBlockingStreamRequest<F>) -> Result<Response<NonBlockingWebStream<NonBlockingStreamRequest<F>, tonic::Streaming<Bytes>>>, Status>
where
