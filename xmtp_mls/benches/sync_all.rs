...
async fn add_to_groups(client: &Client<TestApiClient>, num_groups: usize) -> Vec<BenchGroup> {
    let mut groups = vec![];
    for _ in 0..num_groups {
        let transient_identity = create_client(false, false).await;
        let group = transient_identity
            .create_group_with_inbox_ids(&[client.inbox_id().to_string()], None, None)
            .await
            .unwrap();
...
fn sync_10_groups_many_messages(c: &mut Criterion) {
    ...
    let groups = runtime.block_on(async {
        let groups = add_to_groups(&client, 10).await;
            ...
            b.to_async(&runtime).iter_batched(
                || {
                    bench_async_setup(|| async {
                        ...
                        let grps = add_to_groups(&client, to_add).await;
                        let groups = {
