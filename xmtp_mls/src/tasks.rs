    ...
    Context: XmtpSharedContext + 'static,
{
    ...
    pub async fn run(&mut self) -> Result<(), TaskWorkerError> {
        ...
        loop {
            ...
            tokio::select! {
                ...
                () = tokio::time::sleep_until(next_wakeup) => {
    ...
    fn next_wakeup(
        ...
        if now > next_task_wakeup {
            ...
            tokio_now
                // next_task_wakeup - now is always positive
                .checked_add(tokio::time::Duration::from_nanos(
                    (next_task_wakeup - now) as u64,
                ))
                .expect("next_task_wakeup overflow")
        }
    }
    async fn process_welcome_pointer(
        ...
        let resumable_welcome_pointer =
            crate::groups::welcome_sync::ResumableWelcomeMessage::DecryptedWelcomePointer {
                id: task.originating_message_id.unwrap_or_default() as u64,
                created_ns: task.created_at_ns as u64,
                decrypted_welcome_pointer: welcome_pointer,
            };
        ...
        let group = welcome_service
            ...
            .process_new_welcome(&resumable_welcome_pointer, false, validator)
            .await?;
