...
impl DbConnection {
    ...
    pub fn get_identity_updates<InboxId: AsRef<str>>(
        ...
        Ok(self.raw_query(false, |conn| query.load::<StoredIdentityUpdate>(conn))?)
    }
    ...
    pub fn get_latest_sequence_id_for_inbox(&self, inbox_id: &str) -> Result<i64, StorageError> {
        ...
        Ok(self.raw_query(false, |conn| query.first::<i64>(conn))?)
    }
    ...
    pub fn get_latest_sequence_id(
        ...
        let result_tuples: Vec<(String, i64)> = self
            .raw_query(false, |conn| query.load::<(String, Option<i64>)>(conn))?
            .into_iter()
