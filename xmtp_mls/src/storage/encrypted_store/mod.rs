...
pub mod private {
        ...
        Db: XmtpDb,
    {
        ...
        /// }).await
        /// ```
        pub async fn transaction_async<T, F, E, Fut>(
            &self,
            provider: &XmtpOpenMlsProviderPrivate<<Db as XmtpDb>::Connection>,
            fun: F,
        ) -> Result<T, E>
            ...
            E: From<diesel::result::Error> + From<StorageError>,
        {
            ...
            // essentially manually cloning Provider
            let connection = DbConnectionPrivate::from_arc_mutex(provider.conn_ref().inner_ref());
            let local_connection = connection.inner_ref();
