...
impl Delete<StoredGroupIntent> for DbConnection {
    ...
    fn delete(&self, key: ID) -> Result<usize, StorageError> {
        Ok(self.raw_query(true, |raw_conn| {
...
impl DbConnection {
    ...
    pub fn find_group_intents(
        ...
        Ok(self.raw_query(false, |conn| query.load::<StoredGroupIntent>(conn))?)
    }
    ...
    pub fn set_group_intent_published(
        ...
        let rows_changed = self.raw_query(true, |conn| {
        ...
        if rows_changed == 0 {
            let already_published = self.raw_query(false, |conn| {
                dsl::group_intents
                    .filter(dsl::id.eq(intent_id))
    ...
    pub fn find_group_intent_by_payload_hash(
        ...
        let result = self.raw_query(false, |conn| {
            dsl::group_intents
