...
impl<ScopedClient: ScopedGroupClient> MlsGroup<ScopedClient> {
    ...
    pub async fn send_message_with_provider(
        ...
        self.context()
            .store()
            .transaction_async(provider, |tx_provider| async move {
                let update_interval_ns = Some(SEND_MESSAGE_UPDATE_INSTALLATIONS_INTERVAL_NS);
                self.maybe_update_installations(tx_provider, update_interval_ns)
                    .await?;

                let message_id = self.prepare_message(message, tx_provider, |now| {
