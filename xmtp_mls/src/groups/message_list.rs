...
use xmtp_db::group_message::{
    ContentType as DbContentType, MsgQueryArgs, RelationQuery, StoredGroupMessage,
};
...
type ReplyRootMap = HashMap<Vec<u8>, DecodedMessage>;
    ...
    Context: XmtpSharedContext,
{
    pub fn find_messages_v2(
        ...
        let inbound_relations_query = RelationQuery::builder()
            .content_types(Some(vec![DbContentType::Reaction]))
            .build()
            .ok();

        let outbound_relations_query = RelationQuery::builder().build().ok();

        let messages_with_relations = conn.get_group_messages_with_relations(
            &self.group_id,
            query,
            inbound_relations_query,
            outbound_relations_query,
        )?;

        // Create a mapping of message IDs to their reactions
        let mut reaction_map = get_reactions(messages_with_relations.inbound_relations);
        // Create a mapping of referenced IDs to the messages they point to
        let reply_root_map = get_reply_roots(messages_with_relations.outbound_relations);

        let messages: Vec<DecodedMessage> = messages_with_relations
            .messages
            .into_iter()
            .filter_map(|stored_message| {
                ...
                decoded.reactions = reaction_map
                ...
                if let MessageBody::Reply(mut reply_body) = decoded.content {
                    let _ = hex::decode(&reply_body.reference_id)
                        ...
                        .inspect(|id| {
                            let in_reply_to = reply_root_map.get(id).cloned();
...
fn get_reply_roots(messages: HashMap<Vec<u8>, StoredGroupMessage>) -> ReplyRootMap {
    messages
        .into_iter()
