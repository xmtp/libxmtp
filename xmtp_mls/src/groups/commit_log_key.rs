...
use xmtp_db::{
    sql_key_store::{SqlKeyStoreError, COMMIT_LOG_SIGNER_PRIVATE_KEY},
...
pub(crate) trait CommitLogKeyCrypto {
    ...
    fn public_key_matches_private_key(public_key: &Vec<u8>, private_key: &Secret) -> bool;
}
...
impl CommitLogKeyCrypto for RustCrypto {
    ...
    fn public_key_matches_private_key(public_key: &Vec<u8>, private_key: &Secret) -> bool {
        ...
        public_key.as_slice() == computed_public_key
    }
}
...
pub(crate) fn get_or_create_signing_key(
    ...
    tracing::warn!(
        ...
        consensus_public_key.map(|key| hex::encode(key)),
