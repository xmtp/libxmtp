...
mod tests {
    ...
    async fn test_archive_timestamps() {
        ...
        let export = {
            let mut file = vec![];
            let mut exporter =
                ArchiveExporter::new(opts, alix.db(), alix.context.api_client.clone(), &key);
    ...
    async fn test_dm_archive() {
        ...
        let export = {
            ...
            let mut exporter =
                ArchiveExporter::new(opts, alix.db(), alix.context.api().clone(), &key);
            exporter.read_to_end(&mut file).await?;
            file
        };
    ...
    async fn test_file_backup() {
        ...
        let mut exporter = ArchiveExporter::new(opts, alix.db(), alix.context.api().clone(), &key);
