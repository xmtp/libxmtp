...
pub(crate) mod tests {
    ...
    async fn test_consent_sync() {
        ...
        amal_b
            .send_sync_request(&amal_b.provider, &Retry::default())
            .await;
        ...
        xmtp_common::wait_for_some(|| async {
            amal_b
                .v1_get_latest_sync_reply(&amal_b.provider, DeviceSyncKind::Consent)
                .await
                .unwrap()
        })
        .await;
        ...
        while bo_consent_with_amal_b.is_none() {
            assert_ok!(amal_b_sync_group.sync_with_conn(&amal_b.provider).await);
            bo_consent_with_amal_b = amal_b
                .provider
                .conn_ref()
                .get_consent_record(bo_wallet.get_inbox_id(0), ConsentType::InboxId)
                .unwrap();
