...
pub struct ValidatedCommit {
    ...
    pub inboxes_added: Vec<Inbox>,
    pub inboxes_removed: Vec<Inbox>,
}
...
impl ValidatedCommit {
    pub async fn from_staged_commit<'client, ApiClient: XmtpMlsClient + XmtpIdentityClient>(
        ...
        let (new_group_membership, expected_installation_diff, inboxes_added, inboxes_removed) =
        ...
        let (added_installations, removed_installations, mut credentials_to_verify) =
...
/// A tuple containing all the changes in the commit
/// Contains:
/// 1. The installation_ids added
/// 2. The installation_ids removed
/// 3. The [`CommitParticipant`]s who are being updated through update proposals
type GetProposalChangesResult = (HashSet<Vec<u8>>, HashSet<Vec<u8>>, Vec<CommitParticipant>);

fn get_proposal_changes(
    staged_commit: &StagedCommit,
    openmls_group: &OpenMlsGroup,
) -> Result<GetProposalChangesResult, CommitValidationError> {
...
) -> Result<(GroupMembership, InstallationDiff, Vec<Inbox>, Vec<Inbox>), CommitValidationError> {
