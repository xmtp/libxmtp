    ...
    Context: XmtpSharedContext,
{
    ...
    group_1
        ...
        .unwrap();
    group_2.sync().await.unwrap();
    return group_2.group_name().unwrap() != target_group_name;
}
...
async fn test_fork_recovery_e2e() {
    ...
    // We need a published commit to happen to detect the fork (fork_group() does not publish any commit)
    // (In future, we could remove this requirement by having Bo check whether their epoch authenticator matches their local commit log)
    a_group
    ...
    a_worker._tick().await.unwrap();
    tracing::warn!(
        "(Unrelated) Adding an extra bad commit to test the behavior after Bo's group is recovered"
    );
    a_group
    ...
    tracing::warn!("Confirm B is no longer forked");
    assert!(!is_forked(&a_group, &b_group).await);
    ...
    tracing::warn!("A uploads the previous commit that B couldn't read to the remote log");
    a_worker._tick().await.unwrap();
    tracing::warn!(
        "B pulls down that previous bad commit. They shouldn't conclude that they are forked"
    );
    b_worker._tick().await.unwrap();
    assert!(
        !b_group
