...
use crate::groups::{GroupError, InitialMembershipValidator, MlsGroup};
...
mod tests {
    ...
    async fn happy_path(context: NewMockContext) {
        ...
        let (context, validator) = TestWelcomeSetup::builder()
            ...
            .nested_transaction_calls(|db: &mut MockDbQuery| {
                ...
                db.expect_insert_or_replace_group().returning(|g| Ok(g));
            })
            .mem(mem)
            .build();
    ...
    #[xmtp_common::test]
    async fn does_not_increment(context: NewMockContext, #[case] validator: impl ValidateGroupMembership, #[case] cursor_increment: bool) {
        let mem = Arc::new(SqlKeyStore::new(MemoryStorage::default()));
        let client = create_mls_client(mem.as_ref());
        let (kp, mls_welcome) = client.join_group();
        let network_welcome =
