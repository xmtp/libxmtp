    ...
    async fn process_message_inner(
        ...
        match intent {
            ...
            Some(intent) => {
                ...
                provider.transaction::<_, _, GroupMessageProcessingError>(|provider| {
                    let requires_processing = if allow_cursor_increment {
                        tracing::info!(
                            "calling update cursor for group {}, with cursor {}, allow_cursor_increment is true",
                            hex::encode(envelope.group_id.as_slice()),
                    ...
                    let result: Result<Option<Vec<u8>>, IntentResolutionError> = match maybe_validated_commit {
                        Err(err) => Err(err),
                        Ok(commit) => {
                            self.process_own_message(&mut mls_group, commit, &intent, &message, envelope)
                        }
                    };
                    let (intent_state, internal_message_id) = match result {
                        Err(err) => {
