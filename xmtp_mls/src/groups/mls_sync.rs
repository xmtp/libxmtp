    ...
    Context: XmtpSharedContext,
{
    ...
    pub(super) async fn send_welcomes(
        ...
        let welcome_pointer = if wp_capable > 2 {
            tracing::debug!(
                wp_capable,
                "Using welcome pointers because there are more than 2 capable installations"
            );
            ...
            Some((welcome_pointer_bytes, welcome_pointee))
        } else {
            None
        };
        ...
        let welcomes = action
            ...
            .map(
                |installation| -> Result<WelcomeMessageInput, WrapWelcomeError> {
                    ...
                    if let Some(welcome_pointer) = &welcome_pointer
                        && wp_cap.compatible()
                    {
