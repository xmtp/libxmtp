...
impl MembershipIntentData {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, IntentError> {
        ...
        } = UpdateGroupMembershipData::decode(bytes.as_slice())?
        {
            Ok(Self::new(
                v1.added_members,
                v1.removed_members,
    ...
    fn apply_failed_installations(&self, group_membership: &GroupMembership) -> GroupMembership {
        let applied = group_membership.clone();
        applied.failed_installations = group_membership
            .failed_installations
            .into_iter()
...
impl UpdateGroupMembershipIntent {
    ...
    pub async fn new(
        ...
        .await?;
        data.failed_installations = changes_with_kps.failed_installations;
        ...
        {
            return Err(IntentError::FailedToVerifyInstallations);
        }
...
impl GroupIntent for UpdateGroupMembershipIntent {
    async fn publish_data(
        ...
        let membership_diff = self.old.diff(&self.new_membership);
        ...
        new_extensions.add_or_replace(build_group_membership_extension(&self.new_group_membership));
    ...
    fn from(intent: UpdateGroupMembershipIntent) -> Self {
        ...
        UpdateGroupMembershipData {
            version: Some(UpdateGroupMembershipVersion::V1(UpdateGroupMembershipV1 {
                added_members: intent.added_members,
