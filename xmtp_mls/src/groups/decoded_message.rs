...
use xmtp_content_types::membership_change::GroupMembershipChangeCodec;
...
impl TryFrom<EncodedContent> for MessageBody {
    ...
    fn try_from(value: EncodedContent) -> Result<Self, Self::Error> {
        ...
        match (content_type.type_id.as_str(), content_type.version_major) {
            ...
            (GroupMembershipChangeCodec::TYPE_ID, GroupMembershipChangeCodec::MAJOR_VERSION) => {
                let group_membership_change = GroupMembershipChangeCodec::decode(value)?;
                Ok(MessageBody::GroupMembershipChanges(group_membership_change))
            }
