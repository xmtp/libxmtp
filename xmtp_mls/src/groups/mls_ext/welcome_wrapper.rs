...
pub fn wrap_welcome(
    ...
    match wrapper_algorithm {
        ...
        WrapperAlgorithm::XWingMLKEM512 => wrap_welcome_inner(
...
pub fn unwrap_welcome(
    ...
    match wrapper_algorithm {
        ...
        WrapperAlgorithm::XWingMLKEM512 => unwrap_welcome_inner(
            &LibcruxCryptoProvider::default(),
            &ciphertext,
            private_key,
            wrapper_algorithm.to_mls_ciphersuite(),
...
mod tests {
    ...
    async fn round_trip_xwing_mlkem512() {
        ...
        let private_key =
            find_key_package_private_key(&provider, &pq_pub_key, WrapperAlgorithm::XWingMLKEM512);
        let to_encrypt = vec![1, 2, 3];

        let wrapped = wrap_welcome(
            to_encrypt.as_slice(),
            &pq_pub_key,
            &WrapperAlgorithm::XWingMLKEM512,
        )
        .unwrap();

        let unwrapped =
            unwrap_welcome(&wrapped, &private_key, WrapperAlgorithm::XWingMLKEM512).unwrap();
