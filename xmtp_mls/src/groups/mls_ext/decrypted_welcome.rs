...
impl DecryptedWelcome {
    ...
    fn welcome_from_proto_v1(
        ...
        let wrapper_ciphersuite = WrapperAlgorithm::from(*wrapper_algorithm);
        ...
        let welcome_bytes = unwrap_welcome(data, &private_key, wrapper_ciphersuite.clone())?;
        let welcome = deserialize_welcome(&welcome_bytes)?;

        let welcome_metadata = if welcome_metadata.is_empty() {
            tracing::debug!("Welcome Metadata is empty; proceeding without metadata.");
            None
        } else {
            match unwrap_welcome(welcome_metadata, &private_key, wrapper_ciphersuite) {
    ...
    async fn welcome_from_proto_welcome_pointer(
        ...
        let welcome_pointer_v1 = match welcome_pointer.version {
            ...
            None => {
                return Err(GroupError::MissingField("WelcomePointer.version"));
            }
        };
        let crypto_provider = openmls_libcrux_crypto::CryptoProvider::new()
            .expect("Failed to create LibcruxCryptoProvider because of insufficient randomness");
        let aead_type =
        ...
        let aead_type = match aead_type {
            ...
            xmtp_proto::xmtp::mls::message_contents::WelcomePointeeEncryptionAeadType::Unspecified => {
                return Err(GroupError::MissingField("WelcomePointer.version.v1.aead_type"));
            }
        };
        let decrypted_welcome = crypto_provider.aead_decrypt(
            aead_type,
            &welcome_pointer_v1.encryption_key,
            &v1.data,
            &welcome_pointer_v1.nonce,
            &[],
        )?;
        let welcome = deserialize_welcome(&decrypted_welcome)?;
        let welcome_metadata = (!v1.welcome_metadata.is_empty())
            .then(|| {
                let decrypted_welcome_metadata = crypto_provider.aead_decrypt(
                    aead_type,
                    &welcome_pointer_v1.encryption_key,
                    &v1.welcome_metadata,
                    &welcome_pointer_v1.nonce,
                ...
                deserialize_welcome_metadata(&decrypted_welcome_metadata)
            })
            .transpose()?;
...
pub(crate) fn decrypt_welcome_pointer(
    ...
    let private_key =
        find_private_key(provider, &hash_ref, &WrapperAlgorithm::XWingMLKEM768Draft6)?;
