...
impl DecryptedWelcome {
    ...
    async fn welcome_from_proto_welcome_pointer(
        ...
        let welcome_pointer_v1 = match welcome_pointer.version {
            ...
            None => {
                return Err(GroupError::MissingField(
                    "WelcomePointer.version in welcome_from_proto_welcome_pointer",
                ));
            }
        };
        let aead_type =
        ...
        let nonces = [
            welcome_pointer_v1
                .nonces
                .get(0)
                .ok_or(GroupError::MissingField(
                    "WelcomePointer.version.v1.nonces is empty",
                ))?
                .as_slice(),
            welcome_pointer_v1
                .nonces
                .get(1)
                .ok_or(GroupError::MissingField(
                    "WelcomePointer.version.v1.nonces only has 1 nonce",
                ))?
                .as_slice(),
        ];
