    ...
    V: SmartContractSignatureVerifier + Send + Sync + 'static,
{
        ...
        ApiClient: XmtpMlsStreams,
    {
        ...
        let stream = stream.filter_map(move |either| async move {
            tracing::info!(
                inbox_id = self.inbox_id(),
            ...
            let filtered = if let Some(p) = provider {
                self.process_streamed_convo(p, either).await
            } else {
                let provider = self.mls_provider().ok()?;
                self.process_streamed_convo(&provider, either).await
            };
    ...
    async fn process_streamed_convo<'a, 'b>(
        &'a self,
        provider: &'b XmtpOpenMlsProvider,
    ...
    V: SmartContractSignatureVerifier + Send + Sync + 'static,
{
    pub fn stream_conversations_with_callback(
        ...
        crate::spawn(Some(rx), async move {
            let stream = client.stream_conversations(None, conversation_type).await?;
    ...
    ) -> Result<impl Stream<Item = Result<StoredGroupMessage, SubscribeError>> + '_, ClientError>
    {
        ...
        let stream = async_stream::stream! {
            ...
            let convo_stream = self.stream_conversations(Some(&provider), conversation_type).await?;
...
pub(crate) mod tests {
    ...
    async fn test_stream_welcomes() {
        ...
        crate::spawn(None, async move {
            let bob_stream = bob_ptr.stream_conversations(None, None).await.unwrap();
    ...
    async fn test_stream_messages() {
        ...
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        crate::spawn(None, async move {
            let stream = alice_group.stream(None).await.unwrap();
            futures::pin_mut!(stream);
