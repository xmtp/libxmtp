...
use crate::{
    context::XmtpContextProvider, intents::ProcessIntentError, subscriptions::SubscribeError,
};
use crate::{
    ...
    Db: XmtpDb,
{
    async fn process(&self, msg: &group_message::V1) -> Result<MessageIdentifier, SubscribeError> {
        ...
        tracing::debug!(
            "about to process streamed message for group in epoch={} for [{}]",
            epoch,
            self.0.inbox_id()
        );
        group
    ...
    D: GroupDatabase,
{
    ...
    pub(crate) async fn process(
        ...
        if let Some(new_msg) = new_message {
            ...
            let next: Option<u64> = summary.process.first_new().or(Some(msg.id));
            Ok(ProcessedMessage {
                message: None,
                next_message: next,
    ...
    async fn process_or_recover(&self, msg: &group_message::V1) -> SyncSummary {
        ...
        match process_result {
            ...
            Err(ReceiveGroup(_)) => self.syncer.recover(msg).await,
            Err(e) => {
