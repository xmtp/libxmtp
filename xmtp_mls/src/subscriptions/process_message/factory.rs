    ...
    Context: XmtpSharedContext,
{
    fn last_cursor(&self, group_id: &[u8], originator_id: u32) -> Result<Cursor, StorageError> {
        self.0
            .db()
            .get_last_cursor_for_originator(group_id, EntityKind::Group, originator_id)
    }
    ...
    fn msg(
        ...
        .unwrap_or(conn.get_group_message_by_timestamp(&msg.group_id, msg.timestamp()))
    ...
    D: GroupDatabase,
{
    ...
    fn needs_to_sync(&self, msg: &xmtp_proto::types::GroupMessage) -> Result<bool, SubscribeError> {
        ...
        if msg.originator_id() != last_synced_id.originator_id {
            //TODO:d14n turn into error
            //this is for testing/asserting the code here is correct
            panic!("must be the same");
        }
