    ...
    Factory: ProcessFutureFactory<'a> + 'a,
{
    ...
    async fn subscribe(
        ...
        // TODO:d14n check cursors, probvably incorrect + requires cursor store
        Ok((stream, new_group, Some(Cursor {
            sequence_id: 1,
            originator_id: 0
        })))
    }
    ...
    C::ApiClient: XmtpMlsStreams + 'a,
{
    ...
    fn on_waiting(
        ...
        if let Some(position) = cursor {
            if position.last_streamed() > next_msg.cursor && position.started() > next_msg.cursor {
                tracing::warn!(
            ...
            } else if position.last_streamed() > next_msg.cursor && position.started() < next_msg.cursor {
                tracing::debug!(
    ...
    fn resolve_group_additions(mut self: Pin<&mut Self>, group: MlsGroup<C>) {
        ...
        let this = self.as_mut().project();
        // TODO:d14n is it ok to start  at cursor 1 and OID 0, seeing as 0 is the genesis for
        // on-chain
        this.groups.add(&group.group_id, MessagePosition::new(Cursor::new(1, 0u32), Cursor::new(1, 0u32)));
    ...
    fn skip(
        ...
        while let Some(new_envelope) = ready!(self.as_mut().next_message(cx)) {
            ...
            if let Some(stream_cursor) = self.as_ref().groups.position(&new_envelope.group_id) {
                if stream_cursor.last_streamed() > new_envelope.cursor {
                    tracing::debug!(
