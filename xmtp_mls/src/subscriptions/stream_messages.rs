    ...
    C::ApiClient: XmtpMlsStreams + 'a,
{
    ...
    fn on_waiting(
        ...
        if let Some(position) = cursor {
            if position.has_seen(next_msg.cursor) && position.started_after(next_msg.cursor) {
                tracing::warn!(
            ...
            } else if position.has_seen(next_msg.cursor) && position.started_before(next_msg.cursor)
            {
                tracing::debug!(
    ...
    fn skip(
        ...
        while let Some(new_envelope) = ready!(self.as_mut().next_message(cx)) {
            ...
            if let Some(stream_cursor) = self.as_ref().groups.position(&new_envelope.group_id) {
                if stream_cursor.has_seen(envelope.cursor) {
                    tracing::debug!(
