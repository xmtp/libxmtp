...
mod tests {
    ...
    #[timeout(Duration::from_secs(20))]
    #[cfg_attr(target_arch = "wasm32", ignore)]
    async fn test_stream_all_messages_errors() {
        let alix = ClientBuilder::new_test_client(&generate_local_wallet()).await;
        let bo = ClientBuilder::new_test_client(&generate_local_wallet()).await;
        let caro_wallet = generate_local_wallet();
        let caro = ClientBuilder::new_test_client(&caro_wallet).await;
        ...
        bo_group.send_message(b"second").await.unwrap();
        // let bo_conversations = bo.list_conversations()
        // println!("bo group id:{:?}", ethers::utils::hex::encode(&bo_conversation.conversation.inner.group_id));
        // bo_conversation.conversation.send("2 from bo".as_bytes().to_vec()).await.unwrap();
        // bo_conversation.update_group_name("alix update group name 1".to_string()).await.unwrap();
        // bo_conversation.update_group_name("alix update group name 1".to_string()).await.unwrap();
        // bo_conversation.update_group_name("alix update group name 1".to_string()).await.unwrap();
        // bo_conversation.update_group_name("alix update group name 1".to_string()).await.unwrap();
        // bo_conversation.update_group_name("alix update group name 1".to_string()).await.unwrap();
