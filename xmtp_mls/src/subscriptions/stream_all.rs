...
mod tests {
    ...
    async fn stream_messages_keeps_track_of_cursor() {
        ...
        let alice = Arc::new(ClientBuilder::new_test_client(&wallet).await);
        let bob = ClientBuilder::new_test_client(&generate_local_wallet()).await;
        let eve = ClientBuilder::new_test_client(&generate_local_wallet()).await;
        let group = alice
            .create_group(None, GroupMetadataOptions::default())
            .unwrap();
        ...
        let alice_2 = ClientBuilder::new_test_client(&wallet).await;
