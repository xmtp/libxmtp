...
mod tests {
    ...
    pub async fn test_process_returns_correct_cursor_on_err(
        ...
        let current_message = generate_message(
            *success.first().unwrap_or(&55),
            &GroupId::generate().to_vec(),
        );
        let mut mock_syncer = MockSync::new();
        let mut mock_db = MockGroupDatabase::new();
        // the last cursor is
        ...
        // TODO:d14n for this just turn numbers into Cursor with a const originator_id
        mock_syncer
            .expect_recover()
            .times(1)
    ...
    pub async fn test_cursor_no_sync(#[case] message: Option<StoredGroupMessage>) {
        let current_message = generate_message(55, &vec![0]);
