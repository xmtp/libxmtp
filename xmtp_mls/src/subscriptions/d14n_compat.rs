...
enum D14nCompatDecodeError {
    ...
    // indicates a bug in the GroupMessage/WelcomeMessage extractor
    #[error("{0} is missing from deserialized streamed message")]
    MessageMissing(&'static str),
}
...
impl D14nCompatDecodeError {
    ...
    fn missing<T>() -> SubscribeError {
        SubscribeError::dyn_err(D14nCompatDecodeError::MessageMissing(
            std::any::type_name::<T>(),
        ))
    }
}
