...
use xmtp_proto::types::{WelcomeMessage, Cursor};
    ...
    C::Db: 'a,
{
    ...
    pub async fn from_cow(
        ...
        let id_cursor = conn.get_last_cursor_for_originator(installation_key, EntityKind::Welcome, Originators::WELCOME_MESSAGES.into())?;
        tracing::debug!(
            cursor = %id_cursor,
        ...
        let subscription = context
            .api()
            .subscribe_welcome_messages(installation_key.as_ref(), Some(id_cursor.sequence_id as u64))
            .await?;
    ...
    Subscription: Stream<Item = Result<WelcomeOrGroup>> + 'static,
{
    ...
    fn filter_welcome(
        ...
        match welcome {
            ...
            Ok(ProcessWelcomeResult::NewStored { group, maybe_sequence_id, maybe_originator }) => {
                tracing::debug!(
                    group_id = hex::encode(&group.group_id),
                    "finished processing with group {}",
                    hex::encode(&group.group_id)
                );
                if let Some(id) = maybe_sequence_id && let Some(originator) = maybe_originator {
